...
public:
    ...
    using ConnectionCallback = std::function<void(ConnectionId, ConnectionEvent)>;
    ...
    ConnectionInfo* GetConnectionByAddress(const RakNet::SystemAddress& address);
    
    // Get all connections
    const std::unordered_map<ConnectionId, ConnectionInfo>& GetAllConnections() const;
    ...
    void OnConnectionEvent(const RakNet::SystemAddress& address, ConnectionEvent event);

private:
    RakNet::RakPeerInterface* _peer;
    std::unordered_map<ConnectionId, ConnectionInfo> _connections;
    // Custom hash and equal functions for the map since RakNet::SystemAddress doesn't have a proper hash implementation
    std::map<std::string, ConnectionId> _addressToId; // Use string representation as key
