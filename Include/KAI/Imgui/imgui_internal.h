...
// don't provide any guarantee of forward compatibility! Implement maths
// operators for ImVec2 (disabled by default to not collide with using
...
extern IMGUI_API ImGuiContext*
    GImGui;  // Current implicit ImGui context pointer
#endif
...
IMGUI_API bool ImIsPointInTriangle(const ImVec2& p, const ImVec2& a,
...
IMGUI_API int ImFormatString(char* buf, int buf_size, const char* fmt, ...)
    IM_PRINTFARGS(3);
IMGUI_API int ImFormatStringV(char* buf, int buf_size, const char* fmt,
                              va_list args);
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_PressedOnClickRelease =
        1 << 1,  // (default) return pressed on click+release on same item
                 // (default if no PressedOn** flag is set)
    ...
    ImGuiButtonFlags_PressedOnDoubleClick =
        1 << 4,  // return pressed on double-click (default requires
                 // click+release)
    ...
    ImGuiButtonFlags_AlignTextBaseLine =
        1 << 8,  // vertically align button to match text baseline - ButtonEx()
                 // only
    ImGuiButtonFlags_NoKeyModifiers =
        1 << 9,  // disable interaction if a key modifier is held
    ImGuiButtonFlags_AllowOverlapMode =
        1 << 10  // require previous frame HoveredId to either match id or be
...
struct IMGUI_API ImGuiTextEditState {
    ...
    int CurLenA, CurLenW;  // we need to maintain our buffer length in both
                           // UTF-8 and wchar format.
...
struct ImGuiPopupRef {
    ...
    ImVec2
        MousePosOnOpen;  // Copy of mouse position at the time of opening popup

    ImGuiPopupRef(ImGuiID id, ImGuiWindow* parent_window,
                  ImGuiID parent_menu_set, const ImVec2& mouse_pos) {
        PopupId = id;
        Window = NULL;
...
struct ImGuiContext {
    ...
    float FontBaseSize;  // (Shortcut) == IO.FontGlobalScale * Font->Scale *
                         // Font->FontSize. Size of characters.
    ...
    ImGuiWindow*
        MovedWindow;  // Track the child window we clicked on to move a window.
    ...
    ImRect SetNextWindowSizeConstraintRect;  // Valid if
                                             // 'SetNextWindowSizeConstraint' is
                                             // true
    ...
    ImGuiID ScalarAsInputTextId;  // Temporary text input when CTRL+clicking on
                                  // a slider, etc.
    ImGuiStorage
        ColorEditModeStorage;  // Store user selection of color edit mode
    ...
    ImVec2 ScrollbarClickDeltaToGrabCenter;  // Distance between mouse and
                                             // center of grab box, normalized
                                             // in parent space. Use storage?
    char Tooltip[1024];
    char* PrivateClipboard;  // If no custom clipboard handler is defined
    ImVec2 OsImePosRequest,
        OsImePosSet;  // Cursor position request & last passed to the OS Input
                      // Method Editor
    ...
    // Misc
    float
        FramerateSecPerFrame[120];  // calculate estimate of framerate for user
    int FramerateSecPerFrameIdx;
    float FramerateSecPerFrameAccum;
    int CaptureMouseNextFrame;  // explicit capture via CaptureInputs() sets
                                // those flags
...
struct IMGUI_API ImGuiDrawContext {
    ...
    ImVec2 CursorMaxPos;  // Implicitly calculate the size of our contents,
                          // always extending. Saved into window->SizeContents
    ...
    ImGuiID LastItemId;
    ImRect LastItemRect;
    bool LastItemHoveredAndUsable;  // Item rectangle is hovered, and its window
                                    // is currently interactable with (not
                                    // blocked by a popup preventing access to
                                    // the window)
    bool LastItemHoveredRect;  // Item rectangle is hovered, but its window may
                               // or not be currently interactable with (might
                               // be blocked by a popup preventing access to the
                               // window)
    ...
    bool AllowKeyboardFocus;  // == AllowKeyboardFocusStack.back() [empty ==
                              // true]
...
struct IMGUI_API ImGuiWindow {
    ...
    int IndexWithinParent;  // Order within immediate parent window, if we are a
    ...
    int HiddenFrames;
    int SetWindowPosAllowFlags;   // bit ImGuiSetCond_*** specify if
                                  // SetWindowPos() call will succeed with this
                                  // particular flag.
    int SetWindowSizeAllowFlags;  // bit ImGuiSetCond_*** specify if
                                  // SetWindowSize() call will succeed with this
                                  // particular flag.
    int SetWindowCollapsedAllowFlags;  // bit ImGuiSetCond_*** specify if
                                       // SetWindowCollapsed() call will succeed
    ...
    ImGuiSimpleColumns
        MenuColumns;  // Simplified columns storage for menu items
    ImGuiStorage StateStorage;
    float FontWindowScale;  // Scale multiplier per-window
    ImDrawList* DrawList;
    ImGuiWindow*
        RootWindow;  // If we are a child window, this is pointing to the first
                     // non-child parent window. Else point to ourself.
    ImGuiWindow*
        RootNonPopupWindow;     // If we are a child window, this is pointing to
                                // the first non-child non-popup parent window.
                                // Else point to ourself.
    ImGuiWindow* ParentWindow;  // If we are a child window, this is pointing to
                                // our parent window. Else point to NULL.
...
IMGUI_API void
EndFrame();  // Ends the ImGui frame. Automatically called by Render()! you most
             // likely don't need to ever call that yourself directly. If you
             // don't need to render you can call EndFrame() but you'll have
             // wasted CPU already. If you don't need to render, don't create
             // any windows instead!
...
IMGUI_API bool IsHovered(const ImRect& bb, ImGuiID id,
...
// FIXME: All those functions are a mess and needs to be refactored into
// something decent. AVOID USING OUTSIDE OF IMGUI.CPP! NOT FOR PUBLIC
// CONSUMPTION. We need: a sort of symbol library, preferably baked into font
// atlas when possible + decent text rendering helpers.
...
IMGUI_API void RenderCollapseTriangle(ImVec2 pos, bool is_open,
                                      float scale = 1.0f);
...
IMGUI_API bool SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v,
                              float v_min, float v_max, float power,
                              int decimal_precision,
                              ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderFloatN(const char* label, float* v, int components,
                            float v_min, float v_max,
                            const char* display_format, float power);
IMGUI_API bool SliderIntN(const char* label, int* v, int components, int v_min,
                          int v_max, const char* display_format);

IMGUI_API bool DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v,
                            float v_speed, float v_min, float v_max,
                            int decimal_precision, float power);
IMGUI_API bool DragFloatN(const char* label, float* v, int components,
                          float v_speed, float v_min, float v_max,
                          const char* display_format, float power);
IMGUI_API bool DragIntN(const char* label, int* v, int components,
...
IMGUI_API bool InputIntN(const char* label, int* v, int components,
                         ImGuiInputTextFlags extra_flags);
IMGUI_API bool InputScalarEx(const char* label, ImGuiDataType data_type,
                             void* data_ptr, void* step_ptr,
                             void* step_fast_ptr, const char* scalar_format,
                             ImGuiInputTextFlags extra_flags);
...
IMGUI_API bool TreeNodeBehaviorIsOpen(
    ImGuiID id, ImGuiTreeNodeFlags flags =
                    0);  // Consume previous SetNextTreeNodeOpened() data, if
