...
#ifdef _WIN32
#define IM_NEWLINE \
    "\r\n"  // Play it nice with Windows users (2018/05 news: Microsoft
            // announced that Notepad will finally display Unix-style carriage
            // returns!)
#else
#define IM_NEWLINE "\n"
#endif
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_DontClosePopups =
        1 << 7,  // disable automatically closing parent popup on press //
                 // [UNUSED]
...
enum ImGuiColumnsFlags_ {
    ...
    ImGuiColumnsFlags_GrowParentContentsSize =
        1 << 4  // (WIP) Restore pre-1.51 behavior of extending the parent
                // window contents size but _without affecting the columns width
                // at all_. Will eventually remove.
};
...
struct IMGUI_API ImGuiInputTextState {
    ...
    ImVector<char> TempBuffer;  // temporary buffer for callback and other other
                                // operations. size=capacity.
...
struct ImGuiContext {
    ...
    bool ActiveIdAllowOverlap;  // Active widget allows another widget to steal
                                ...
                                // but not always)
    bool ActiveIdHasBeenEdited;  // Was the value associated to the widget
                                 // Edited over the course of the Active state.
    ...
    ImGuiInputSource
        NavInputSource;  // Keyboard or Gamepad mode? THIS WILL ONLY BE None or
                         // NavGamepad or NavKeyboard.
    ImRect
        NavScoringRectScreen;  // Rectangle used for scoring, in screen space.
                               // Based of window->DC.NavRefRectRel[], modified
    ...
    ImGuiNavLayer NavLayer;  // Layer we are navigating on. For now the system
                             // is hard-coded for 0=main contents and
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_ButtonRepeat =
        1 << 1,  // false    // Button() will return true multiple times based
                 // on io.KeyRepeatDelay and io.KeyRepeatRate settings.
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImGuiItemFlags ItemFlags;  // == ItemFlagsStack.back() [empty ==
                               // ImGuiItemFlags_Default]
...
struct IMGUI_API ImGuiWindow {
    ...
    ImVec2 SizeFullAtLastBegin;  // Copy of SizeFull at the end of Begin. This
                                 // is the reference value we'll use on the next
    ...
    short BeginOrderWithinParent;   // Order within immediate parent window, if
                                    // we are a child window. Otherwise 0.
    short BeginOrderWithinContext;  // Order within entire imgui context. This
                                    // is mostly used for debugging submission
    ...
    ImGuiCond
        SetWindowCollapsedAllowFlags;  // store acceptable condition flags for
    ...
    ImRect ContentsRegionRect;  // FIXME: This is currently
                                // confusing/misleading. Maximum visible content
                                // position ~~ Pos + (SizeContentsExplicit ?
    ...
    int SettingsIdx;  // Index into SettingsWindow[] (indices are always valid
    ...
    ImGuiWindow* RootWindow;  // Point to ourself or first ancestor that is not
                              // a child window.
    ImGuiWindow*
...
IMGUI_API int CalcTypematicPressedRepeatAmount(float t, float t_prev,
                                               float repeat_delay,
                                               float repeat_rate);
...
inline bool IsNavInputPressedAnyOfTwo(ImGuiNavInput n1, ImGuiNavInput n2,
                                      ImGuiInputReadMode mode) {
...
IMGUI_API void RenderColorRectWithAlphaCheckerboard(
    ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, float grid_step,
    ImVec2 grid_off, float rounding = 0.0f, int rounding_corners_flags = ~0);
...
IMGUI_API void RenderMouseCursor(
    ImDrawList* draw_list, ImVec2 pos, float scale,
...
IMGUI_API bool DragBehavior(ImGuiID id, ImGuiDataType data_type, void* v,
                            float v_speed, const void* v_min, const void* v_max,
...
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed,
                             const T v_min, const T v_max, const char* format,
                             float power, ImGuiDragFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(const ImRect& bb, ImGuiID id,
                               ImGuiDataType data_type, T* v, const T v_min,
                               const T v_max, const char* format, float power,
                               ImGuiSliderFlags flags, ImRect* out_grab_bb);
template <typename T, typename FLOAT_T>
IMGUI_API float SliderCalcRatioFromValueT(ImGuiDataType data_type, T v, T v_min,
                                          T v_max, float power,
...
IMGUI_API bool InputTextEx(const char* label, char* buf, int buf_size,
                           ...
                           void* user_data = NULL);
IMGUI_API bool InputScalarAsWidgetReplacement(const ImRect& bb, ImGuiID id,
                                              const char* label,
...
IMGUI_API void ImFontAtlasBuildMultiplyRectAlpha8(
    const unsigned char table[256], unsigned char* pixels, int x, int y, int w,
    int h, int stride);
