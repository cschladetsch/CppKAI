...
ImGuiPayload)
// Obsolete functions
// Helpers (ImVector, ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer,
...
// Configuration file (edit imconfig.h or define IMGUI_USER_CONFIG to your own
// filename)
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open =
        NULL);  // create metrics window. display Dear ImGui internals: draw
                // commands (with individual draw calls and vertices), window
                // list, basic internal state, etc.
...
// upper-right corner of the window, which clicking will set the boolean to
// false when clicked.
...
//   return value [this is due to legacy reason and is inconsistent with most
//   other functions such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where
//   the EndXXX call should only be called if the corresponding BeginXXX
...
// or your own, see misc/cpp/imgui_stdlib.h
...
// work-in-progress and rather lacking (columns are arguably the worst part of
...
GetDragDropPayload();  // peek directly into the current payload from anywhere.
                       // may return NULL. use ImGuiPayload::IsDataType() to
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_CallbackResize =
        1
        << 18,  // Callback on buffer capacity changes request (beyond
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1 << 13,  // (WIP) Nav: left direction may move to this TreeNode() from
                  // any of its child (items submitted between TreeNode and
                  // TreePop)
...
enum ImGuiSelectableFlags_ {
    ...
    ImGuiSelectableFlags_SpanAllColumns =
        1 << 1,  // Selectable frame can span all columns (text will still fit
                 // in current column)
...
enum ImGuiNavInput_ {
    ...
    ImGuiNavInput_LStickLeft,  // scroll / move window (w/ PadMenu) // e.g. Left
                               // Analog Stick Left/Right/Up/Down
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableGamepad =
        1 << 1,  // Master gamepad navigation enable flag. This is mostly to
                 // instruct your imgui back-end to fill io.NavInputs[].
    ...
    ImGuiConfigFlags_NoMouse =
        1 << 4,  // Instruct imgui to clear mouse position/buttons in
                 // NewFrame(). This allows ignoring the mouse information set
                 // by the back-end.
...
struct ImGuiStyle {
    ...
    ImVec2 DisplaySafeAreaPadding;  // If you cannot see the edges of your
...
struct ImGuiIO {
    ...
    ImGuiBackendFlags
        BackendFlags;    // = 0                  // See ImGuiBackendFlags_ enum.
                         // Set by back-end (imgui_impl_xxx files or custom
                         // back-end) to communicate features supported by the
                         // back-end.
    ImVec2 DisplaySize;  // <unset>              // Main display size, in
                         // pixels. For clamping windows positions.
    ...
    float MouseDoubleClickMaxDist;  // = 6.0f               // Distance
                                    // threshold to stay in to validate a
    ...
    void* UserData;        // = NULL               // Store your own data for
                           // retrieval by callbacks.
    ...
    ImVec2 DisplayVisibleMin;  // <unset> (0.0f,0.0f)  // [OBSOLETE] If you use
                               // DisplaySize as a virtual space larger than
                               // your screen, set DisplayVisibleMin/Max to the
                               // visible area.
    ImVec2 DisplayVisibleMax;  // <unset> (0.0f,0.0f)  // [OBSOLETE: just use
                               // io.DisplaySize!] If the values are the same,
    ...
    bool ConfigMacOSXBehaviors;  // = defined(__APPLE__) // OS X style: Text
                                 ...
                                 // text, Multi-selection in lists uses
    ...
    ImVec2 MousePos;    // Mouse position, in pixels. Set to
                        // ImVec2(-FLT_MAX,-FLT_MAX) if mouse is unavailable (on
                        // another screen, etc.)
    ...
    ImWchar InputCharacters[16 + 1];  // List of characters input (translated by
                                      // user from keypress+keyboard state).
    ...
    float NavInputs[ImGuiNavInput_COUNT];  // Gamepad inputs (keyboard keys will
                                           ...
                                           // by ImGui::NewFrame, all values
                                           // will be cleared back to zero in
                                           // ImGui::EndFrame)
    ...
    bool WantCaptureMouse;  // When io.WantCaptureMouse is true, imgui will use
                            // the mouse inputs, do not dispatch them to your
                            // main game/application (in both cases, always pass
                            // on mouse inputs to imgui). (e.g. unclicked mouse
                            // is hovering over an imgui window, widget is
                            // active, mouse was clicked over an imgui window,
                            // etc.).
    bool WantCaptureKeyboard;  // When io.WantCaptureKeyboard is true, imgui
                               // will use the keyboard inputs, do not dispatch
                               // them to your main game/application (in both
                               // cases, always pass keyboard inputs to imgui).
                               // (e.g. InputText active, or an imgui window is
...
struct ImGuiListClipper {
    ...
    }  // NB: Begin() initialize every fields (as we allow user to call
       // Begin/End multiple times on a same instance if they want).
    ~ImGuiListClipper() {
...
struct ImDrawList {
    ...
    ImVector<ImDrawChannel>
        _Channels;  // [Internal] draw channels for columns API (not resized
                    // down so _ChannelsCount may be smaller than
                    // _Channels.Size)
...
struct ImDrawData {
    bool Valid;  // Only valid after Render() is called and before the next
                 // NewFrame() is called.
    ImDrawList**
        CmdLists;  // Array of ImDrawList* to render. The ImDrawList are owned
    ...
    int TotalVtxCount;  // For convenience, sum of all ImDrawList's
                        // VtxBuffer.Size
    ImVec2
        DisplayPos;  // Upper-left position of the viewport to render (==
...
struct ImFontConfig {
    ...
    bool PixelSnapH;  // false    // Align every glyph to pixel boundary. Useful
                      // e.g. if you are merging a non-pixel aligned font with
                      // the default font. If enabled, you can set OversampleH/V
                      // to 1.
...
struct ImFontAtlas {
    ...
    ClearInputData();  // Clear input data (all ImFontConfig structures
                       // including sizes, TTF data, glyph ranges, etc.) = all
    ...
    struct CustomRect {
        ...
        unsigned short Width,
            Height;           // Input    // Desired rectangle dimension
    ...
    int TexDesiredWidth;  // Texture width desired by user before Build(). Must
                          // be a power-of-two. If have many glyphs your
                          // graphics API have texture size restrictions you may
...
struct ImFont {
    ...
    ImVector<float>
        IndexAdvanceX;  //              // Sparse. Glyphs->AdvanceX in a
                        //              directly indexable way (more
                        //              cache-friendly, for CalcTextSize
                        //              functions which are often bottleneck in
                        //              large UI).
    ...
    short ConfigDataCount;  // ~ 1          // Number of ImFontConfig involved
                            // in creating this font. Bigger than 1 when merging
                            // multiple font sources into one ImFont.
    ImFontConfig* ConfigData;  //              // Pointer within
                               //              ContainerAtlas->ConfigData
    ImFontAtlas*
        ContainerAtlas;     //              // What we has been loaded into
