...
int main() {
    ...
    try {
        ...
        if (sum == 10) {
            std::cout << "✅ TEST PASSED: Sum is correctly calculated as 10" << std::endl;
        } else {
            std::cout << "❌ TEST FAILED: Expected sum to be 10, but got " << sum << std::endl;
        }

        // Try a more complex for loop with break
        std::cout << "\nExecuting for loop with break condition..." << std::endl;
        ...
        std::cout << "Final value of result: " << exec->GetDataStack()->Top() << std::endl;
        
        // Expected result: result should be 0+1+2+3+4+5=15 (then breaks)
        int result = ConstDeref<int>(exec->GetDataStack()->Top());
        if (result == 15) {
            std::cout << "✅ TEST PASSED: Break condition works correctly" << std::endl;
        } else {
            std::cout << "❌ TEST FAILED: Expected result to be 15, but got " << result << std::endl;
        }
        ...
        std::cout << "Final value of nested_sum: " << exec->GetDataStack()->Top() << std::endl;
        
        // Expected result: Sum of 0 through 8 = 36
        int nested_sum = ConstDeref<int>(exec->GetDataStack()->Top());
        if (nested_sum == 36) {
            std::cout << "✅ TEST PASSED: Nested for loops work correctly" << std::endl;
        } else {
            std::cout << "❌ TEST FAILED: Expected nested_sum to be 36, but got " << nested_sum << std::endl;
        }
