...
// Test 7: Test floating point division and type preservation
TEST(RhoAdvancedOps, FloatDivision) {
    Console console;
    console.SetLanguage(Language::Pi);
    ...
    // Create objects
    Object float1 = reg.New<float>(10.0f);
    Object float2 = reg.New<float>(3.0f);
    
    // Create a continuation that divides float1 by float2
    Object continuation = CreateTestContinuation(reg, {float2, float1}, Operation::Divide);
    ...
    ASSERT_TRUE(stack->Top().IsType<float>()) << "Expected float but got " 
                                            << stack->Top().GetClass()->GetName().ToString();
    
    // Check value (approximately 3.33333)
    ASSERT_NEAR(ConstDeref<float>(stack->Top()), 3.33333f, 0.0001f);
}

// Test 8: Test more complex mixed-type operations
TEST(RhoAdvancedOps, ComplexMixedTypeOps) {
    Console console;
    console.SetLanguage(Language::Pi);
    ...
    // This test simulates: (5 * 2.5) + (10 / 2)
    
    // First calculate 5 * 2.5
    stack->Clear();
    Object five = reg.New<int>(5);
    Object twoPointFive = reg.New<float>(2.5f);
    Object mult = CreateTestContinuation(reg, {five, twoPointFive}, Operation::Multiply);
    exec->Continue(mult);
    
    // Result should be 12.5
    ASSERT_FALSE(stack->Empty());
    ASSERT_TRUE(stack->Top().IsType<float>());
    ASSERT_NEAR(ConstDeref<float>(stack->Top()), 12.5f, 0.0001f);
    ...
    // Finally, add the two results: 12.5 + 5
    stack->Clear();
    Object addition = CreateTestContinuation(reg, {multResult, divResult}, Operation::Plus);
    exec->Continue(addition);
    
    // Final result should be 17.5
    ASSERT_FALSE(stack->Empty());
    ASSERT_TRUE(stack->Top().IsType<float>()) << "Expected float but got " 
                                             << stack->Top().GetClass()->GetName().ToString();
    ASSERT_NEAR(ConstDeref<float>(stack->Top()), 17.5f, 0.0001f);
}
...
// Test 10: Test type preservation with equality comparison across different types
TEST(RhoAdvancedOps, CrossTypeEquality) {
    Console console;
    console.SetLanguage(Language::Pi);
    ...
    // Create objects of different types with "equivalent" values
    Object intVal = reg.New<int>(5);
    Object floatVal = reg.New<float>(5.0f);
    
    // Test equality between int and float
    Object continuation = CreateTestContinuation(reg, {intVal, floatVal}, Operation::Equiv);
    ...
    // The actual result might vary by implementation (some languages consider 5 == 5.0 true, others false)
    // The key point is type preservation of the result as bool
}
...
// Test 12: Test the binary operation of integer and float with non-commutative operations
TEST(RhoAdvancedOps, NonCommutativeOperations) {
    ...
    stack->Clear();
    Object ten = reg.New<int>(10);
    Object threePointFive = reg.New<float>(3.5f);
    Object sub1 = CreateTestContinuation(reg, {threePointFive, ten}, Operation::Minus);
    exec->Continue(sub1);
    
    // Result should be 6.5 (10 - 3.5)
    ASSERT_FALSE(stack->Empty());
    ASSERT_TRUE(stack->Top().IsType<float>());
    ASSERT_NEAR(ConstDeref<float>(stack->Top()), 6.5f, 0.0001f);
    
    // Test float - int
    stack->Clear();
    Object sub2 = CreateTestContinuation(reg, {ten, threePointFive}, Operation::Minus);
    exec->Continue(sub2);
    
    // Result should be -6.5 (3.5 - 10)
    ASSERT_FALSE(stack->Empty());
    ASSERT_TRUE(stack->Top().IsType<float>());
    ASSERT_NEAR(ConstDeref<float>(stack->Top()), -6.5f, 0.0001f);
}
