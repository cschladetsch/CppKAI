 ...
 * -----------------------------------------
 * The Rho language currently has a type mismatch issue when using binary operators
 * like '+', '-', etc. that results in "Type Mismatch: expected=Continuation, got=Signed32"
 * errors. This is likely due to how the translator stacks and manages continuations.
 *
 * Until this is fixed, we're demonstrating basic functionality using Pi language instead,
 * which has a similar model but works correctly. The Do-While loop tests are temporarily
 * disabled until these core type issues are resolved.
 *
 * See the Todo-Rho.md file for detailed information about the issues and planned fixes.
 */
