 ...
 * --------------------------------------
 * This file contains additional binary operation tests that expand test coverage
 * beyond the basic tests. These tests focus on more edge cases, special values,
 * bitwise operations, and complex scenarios.
 */
...
protected:
    ...
    void VerifyIntOperation(Object a, Object b, Operation::Type op, int expected) {
        data_->Clear();
    ...
    void VerifyFloatOperation(Object a, Object b, Operation::Type op, float expected) {
        data_->Clear();
    ...
    void VerifyBoolOperation(Object a, Object b, Operation::Type op, bool expected) {
        data_->Clear();
    ...
    void VerifyStringOperation(Object a, Object b, Operation::Type op, const String& expected) {
        data_->Clear();
    ...
    try {
        ...
        if (data_->Top().IsType<String>()) {
            ASSERT_EQ(ConstDeref<String>(data_->Top()), "Value: 42") 
                << "Expected automatic conversion of int to string for concatenation";
        }
        else {
            // If automatic conversion isn't supported, this test is informational
            std::cout << "String + int automatic conversion not supported" << std::endl;
        }
    }
    catch (const std::exception& e) {
...
TEST_F(ExtendedBinaryOpTests, DivisionByZeroHandling) {
    ...
    try {
        // Should throw an exception
        Object result = exec_->PerformBinaryOp(floatA, floatZero, Operation::Divide);
        data_->Push(result);
    }
    catch (const std::exception& e) {
        exceptionThrown = true;
    ...
    ASSERT_TRUE(exceptionThrown) << "Float division by zero should throw an exception";
}
...
TEST_F(ExtendedBinaryOpTests, LargeNumberOperations) {
    ...
    VerifyIntOperation(large1, large2, Operation::Plus, largeValue1 + largeValue2);
    
    // Subtraction of large numbers
    VerifyIntOperation(large1, large2, Operation::Minus, largeValue1 - largeValue2);
...
TEST_F(ExtendedBinaryOpTests, FloatingPointSpecialValues) {
    ...
    try {
        Object result = exec_->PerformBinaryOp(infObj, normalObj, Operation::Plus);
        data_->Push(result);
        ...
        ASSERT_TRUE(std::isinf(resultValue)) << "Expected infinity but got " << resultValue;
    }
    catch (const std::exception& e) {
    ...
    data_->Clear();
    try {
        Object result = exec_->PerformBinaryOp(nanObj, normalObj, Operation::Plus);
        data_->Push(result);
        ...
        float resultValue = ConstDeref<float>(data_->Top());
        ASSERT_TRUE(std::isnan(resultValue)) << "Expected NaN but got " << resultValue;
    }
    catch (const std::exception& e) {
...
TEST_F(ExtendedBinaryOpTests, StringComparisonOperations) {
    ...
    try {
        VerifyBoolOperation(strA, strB, Operation::Less, true);  // "apple" < "banana"
        VerifyBoolOperation(strB, strA, Operation::Greater, true);  // "banana" > "apple"
    }
    catch (const std::exception& e) {
        std::cout << "String comparison operations not fully supported: " << e.what() << std::endl;
    }
}
...
TEST_F(ExtendedBinaryOpTests, OperatorPrecedenceHandling) {
    ...
    // Step 2: 5 * 4 = 20
    Object altResult = exec_->PerformBinaryOp(altStep1, four, Operation::Multiply);
...
TEST_F(ExtendedBinaryOpTests, EdgeCasesAndBoundaryValues) {
    ...
    try {
        ...
        std::cout << "MAX_INT + 1 = " << ConstDeref<int>(data_->Top()) << std::endl;
    }
    catch (const std::exception& e) {
    ...
    data_->Clear();
    try {
        Object result = exec_->PerformBinaryOp(minObj, oneObj, Operation::Minus);
        data_->Push(result);
        ...
        std::cout << "MIN_INT - 1 = " << ConstDeref<int>(data_->Top()) << std::endl;
    }
    catch (const std::exception& e) {
