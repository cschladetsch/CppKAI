 ...
 * -------------------------
 * This file contains tests specifically focused on Suspend, Resume, and Replace operations
 * within KAI's continuation-based execution model, with special emphasis on their usage
 * within For loops.
 */
...
protected:
    void SetUp() override {
        ...
        // These tests will use Pi language since it gives more direct control over continuations
    ...
    void VerifyStack(const std::vector<std::pair<std::string, int>>& expectedValues) {
        // Make sure we unwrap any continuation values
        UnwrapStackValues(data_, exec_);
        
        ASSERT_EQ(data_->Size(), expectedValues.size()) << "Stack size mismatch";
        ...
        for (size_t i = 0; i < expectedValues.size(); ++i) {
            ...
            if (expected.first == "int") {
                ASSERT_TRUE(actual.IsType<int>()) 
                    ...
                    << (actual.GetClass() ? actual.GetClass()->GetName().ToString() : "<null>");
...
TEST_F(ContinuationControlTests, BasicResumeOperation) {
    // The Resume operation should exit the current continuation and return to the calling context
    data_->Clear();
    
    // Create two continuations:
    // 1. Outer continuation that pushes 1, executes inner continuation, then pushes 3
    // 2. Inner continuation that pushes 2, then calls Resume to exit back to outer continuation
    ...
    // Resume skips the rest of the inner continuation but allows outer to continue
...
TEST_F(ContinuationControlTests, BasicSuspendOperation) {
    // The Suspend operation should suspend the current continuation and start executing a new one
    data_->Clear();
    ...
    // First continuation: Push 1, push second continuation, call Suspend, push 3 (which won't execute yet)
    std::vector<Object> firstCode = {
        reg_->New<Operation>(Operation::ContinuationBegin),
        reg_->New<int>(1),
        secondCont,
        reg_->New<Operation>(Operation::Suspend),
        reg_->New<int>(3),  // This won't execute until the second continuation completes
        reg_->New<Operation>(Operation::ContinuationEnd)
    };
    ...
    // Suspend switches to second continuation, but doesn't complete the first one
    std::vector<std::pair<std::string, int>> expected = {
        {"int", 1},
        {"int", 2}
    };
...
TEST_F(ContinuationControlTests, BasicReplaceOperation) {
    // The Replace operation should replace the current continuation with a new one
    data_->Clear();
    
    // Create two continuations:
    // 1. First continuation that pushes 1, replaces itself with second continuation
    // 2. Second continuation that pushes 2
    ...
    // First continuation: Push 1, push second continuation, call Replace, push 3 (which won't execute)
    std::vector<Object> firstCode = {
        reg_->New<Operation>(Operation::ContinuationBegin),
        reg_->New<int>(1),
        secondCont,
    ...
    // Verify stack: Should contain [1, 2]
    // Replace switches to second continuation without pushing to the context stack
    std::vector<std::pair<std::string, int>> expected = {
        {"int", 1},
        {"int", 2}
    };
    ...
    // Unlike Suspend, Resume after Replace should not continue with the original continuation
    // This might throw an exception if there's no continuation to resume
    try {
        exec_->Resume();
...
TEST_F(ContinuationControlTests, SuspendWithinForLoop) {
    ...
    // Note that 3 appears twice - once before suspension and once after resuming
    expected = {
        {"int", 0},
        {"int", 1},
        {"int", 2},
        {"int", 3},
        {"int", 99},
        {"int", 3},  // Counter value after resuming (dup was already called for 3)
        {"int", 4},
...
TEST_F(ContinuationControlTests, ReplaceWithinForLoop) {
    ...
    try {
        ...
        if (data_->Size() > 6) {
            FAIL() << "For loop continued after Replace, which should not happen";
        }
    }
    catch (const std::exception&) {
        // Expected behavior - no continuation to resume
...
TEST_F(ContinuationControlTests, ConditionalSuspendInForLoop) {
    ...
    // Create a for loop with conditional suspend based on whether the value is even
    const std::string piCode = R"(
    ...
    // Second iteration (i=1) did not trigger suspend, then i=2 triggered suspend
    expected = {
        {"int", 0},
        {"int", 42},
        {"int", 1},
...
// Note: If building these tests is encountering errors, you may need to update the CMakeLists.txt
// file to include this test file in the build process.