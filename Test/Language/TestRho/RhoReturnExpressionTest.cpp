...
TEST_F(RhoReturnExpressionTest, ReturnWithFunctionCall) {
    ...
    ASSERT_EQ(data_->Size(), 1);
    auto result = kai::ConstDeref<int>(data_->Top());
    std::cout << "test_return_func() returned: " << result << " (expected 6)" << std::endl;
    EXPECT_EQ(result, 6);
}
...
TEST_F(RhoReturnExpressionTest, ReturnWithComplexExpression) {
    ...
    ASSERT_EQ(data_->Size(), 1);
    auto result = kai::ConstDeref<int>(data_->Top());
    std::cout << "complex_return(3) returned: " << result << " (expected 10 + 6 + 5 = 21)" << std::endl;
    EXPECT_EQ(result, 21);
}
...
TEST_F(RhoReturnExpressionTest, ReturnOrderAnalysis) {
    ...
    ASSERT_EQ(data_->Size(), 1);
    auto result = kai::ConstDeref<int>(data_->Top());
    std::cout << "test_order() returned: " << result << " (expected 1 + 2 = 3)" << std::endl;
    
    // Check final counter value
    data_->Clear();
    console_.Execute("counter");
    auto counter = kai::ConstDeref<int>(data_->Top());
    std::cout << "Final counter value: " << counter << " (expected 2)" << std::endl;
}