 ...
 * -------------------------
 * This file contains additional tests for Pi and Rho languages to expand test coverage
 * beyond the basic tests. These tests focus on more complex scenarios and edge cases.
 */
...
protected:
    ...
    Object CreateDictionary(const std::map<String, int>& entries) {
        ...
        for (const auto& entry : entries) {
            map->Insert(reg_->New<String>(entry.first), reg_->New<int>(entry.second));
        }
        return map;
    }
    
    // Verify a stack value against expected numerical value with automatic type conversion
    void VerifyStackValue(Object expected) {
        ...
        if (expected.IsType<int>() && actual.IsType<int>()) {
            ...
            ASSERT_NEAR(ConstDeref<float>(actual), ConstDeref<float>(expected), 0.0001) 
            ...
            ASSERT_TRUE(false) << "Type mismatch: Expected " 
                << expected.GetClass()->GetName().ToString() 
                << " but got " << actual.GetClass()->GetName().ToString();
        }
    }
    ...
    void ExecutePiSequence(const std::string& piCode, Object expectedResult) {
        ...
        ASSERT_TRUE(data_->Top().GetClass()->GetName() == expectedResult.GetClass()->GetName()) 
        ...
        if (expectedResult.IsType<int>()) {
            ASSERT_EQ(ConstDeref<int>(data_->Top()), ConstDeref<int>(expectedResult))
                << "Expected value mismatch";
            KAI_TRACE() << "Test for '" << piCode << "' pattern completed successfully with result: " 
                << ConstDeref<int>(data_->Top());
        }
        else if (expectedResult.IsType<float>()) {
            ASSERT_NEAR(ConstDeref<float>(data_->Top()), ConstDeref<float>(expectedResult), 0.0001)
                << "Expected value mismatch";
        }
        else if (expectedResult.IsType<bool>()) {
            ASSERT_EQ(ConstDeref<bool>(data_->Top()), ConstDeref<bool>(expectedResult))
                << "Expected value mismatch";
        }
        else if (expectedResult.IsType<String>()) {
            ASSERT_EQ(ConstDeref<String>(data_->Top()), ConstDeref<String>(expectedResult))
                << "Expected value mismatch";
        }
    }
};
...
TEST_F(AdvancedRhoPiTests, NestedStackRotate) {
    ...
    ASSERT_EQ(data_->Size(), 3) << "Stack should still have 3 elements after rotation";
    ASSERT_EQ(ConstDeref<int>(data_->At(0)), 2) << "New bottom element should be 2";
    ASSERT_EQ(ConstDeref<int>(data_->At(1)), 3) << "New middle element should be 3";
    ASSERT_EQ(ConstDeref<int>(data_->At(2)), 1) << "New top element should be 1";
...
TEST_F(AdvancedRhoPiTests, ArrayOperations) {
    ...
    ASSERT_TRUE(data_->Top().IsType<Array>()) << "Expected Array but got " 
    ...
    ASSERT_EQ(ConstDeref<int>(resultArray->At(0)), 10) << "First element should be 10";
    ASSERT_EQ(ConstDeref<int>(resultArray->At(1)), 20) << "Second element should be 20";
    ASSERT_EQ(ConstDeref<int>(resultArray->At(2)), 30) << "Third element should be 30";
...
TEST_F(AdvancedRhoPiTests, MixedTypeOperations) {
    ...
    try {
        ...
        Object result = exec_->PerformBinaryOp(intObj, floatObj, Operation::Plus);
        data_->Push(result);
        
        cout << "Mixed type operation: 10 + 2.5 = " << ConstDeref<float>(result) << endl;
    }
    catch (const std::exception& e) {
    ...
    UnwrapStackValues(data_, exec_);
    ASSERT_FALSE(data_->Empty()) << "Stack should not be empty";
    ASSERT_TRUE(data_->Top().IsType<float>()) << "Expected float but got " 
                                       << data_->Top().GetClass()->GetName().ToString();
    ASSERT_NEAR(ConstDeref<float>(data_->Top()), 12.5f, 0.0001) << "Expected 10+2.5=12.5";
...
TEST_F(AdvancedRhoPiTests, MultipleOperationSequence) {
    ...
    try {
        ...
        // Step 2: 15 * 2 = 30
        Object step2 = exec_->PerformBinaryOp(
            step1, reg_->New<int>(2), Operation::Multiply);
        ...
        // Step 4: 30 - 4 = 26
        Object result = exec_->PerformBinaryOp(
            step2, step3, Operation::Minus);
        
        data_->Push(result);
        KAI_TRACE() << "Multiple operation sequence successful: (10 + 5) * 2 - 8 / 2 = " 
...
TEST_F(AdvancedRhoPiTests, ErrorHandling) {
    ...
    try {
        ...
        cout << "Expected exception caught during division by zero: " << e.what() << endl;
    }
    
    // Verify exception was thrown
    ASSERT_TRUE(exceptionThrown) << "Division by zero should throw an exception";
...
TEST_F(AdvancedRhoPiTests, BooleanLogicOperations) {
    ...
    try {
        ...
        Object step1 = exec_->PerformBinaryOp(trueObj, falseObj, Operation::LogicalAnd);
        
        // Step 2: true && true = true
        Object step2 = exec_->PerformBinaryOp(trueObj, trueObj, Operation::LogicalAnd);
        
        // Step 3: false || true = true
        Object result = exec_->PerformBinaryOp(step1, step2, Operation::LogicalOr);
        ...
        cout << "Exception during boolean logic operation: " << e.what() << endl;
    ...
    ASSERT_TRUE(data_->Top().IsType<bool>()) << "Expected bool but got " 
                                       << data_->Top().GetClass()->GetName().ToString();
    ASSERT_EQ(ConstDeref<bool>(data_->Top()), true) << "Expected (true && false) || (true && true) = true";
...
TEST_F(AdvancedRhoPiTests, DictionaryOperations) {
    ...
    ASSERT_FALSE(data_->Empty()) << "Stack should not be empty";
    ASSERT_TRUE(data_->Top().IsType<Map>()) << "Expected Map but got " 
                                      << data_->Top().GetClass()->GetName().ToString();
