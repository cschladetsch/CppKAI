...
TEST(RhoPiWorkaround, ArrayOperations) {
    ...
    // Access array elements (Pi doesn't support direct indexing, but we can simulate it)
    stack->Clear();
...
TEST(RhoPiWorkaround, ConditionalLogic) {
    ...
    // Nested condition (similar to: if x > y then if x > 15 then 1 else 2 else 0)
    stack->Clear();
...
TEST(RhoPiWorkaround, FunctionOperations) {
    ...
    // Define a function to add two numbers (equivalent to 'fun add(a, b) { return a + b; }')
    console.Execute("{ + } 'add' !");
    ...
    // Define a more complex function (equivalent to 'fun square(n) { return n * n; }')
    ...
    // Define a function that calls other functions (equivalent to 'fun addSquares(a, b) { return square(a) + square(b); }')
