...
TEST(LanguageComparison, ArithmeticRhoVsPi) {
    ...
    try {
        ...
        try {
            ...
            ASSERT_EQ(ConstDeref<int>(piResult), 5) << "Expected value 5 but got " << piResult.ToString();
    ...
    try {
        ...
        try {
            ...
            ASSERT_TRUE(rhoResult.IsType<int>()) << "Expected int but got " 
                << (rhoResult.Exists() ? rhoResult.GetClass()->GetName().ToString() : "non-existent object");
            ASSERT_EQ(ConstDeref<int>(rhoResult), 5) << "Expected value 5 but got " << rhoResult.ToString();
            
            cout << "Rho test passed: 2 + 3 = " << ConstDeref<int>(rhoResult) << endl;
        }
        catch (const Exception::Base& e) {
            cerr << "Inner KAI Exception: " << e.ToString() << endl;
