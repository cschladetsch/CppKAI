...
protected:
    ...
    void ExecuteAndVerifyArray(const std::string& rhoCode, const std::vector<int>& expectedValues) {
        data_->Clear();
        ...
        try {
            ...
            ASSERT_EQ(array->Size(), expectedValues.size()) << "Array size mismatch";
            
            for (size_t i = 0; i < expectedValues.size(); ++i) {
                ASSERT_TRUE(array->At(i).IsType<int>()) << "Expected int at index " << i;
...
TEST_F(AdvancedRhoLanguage, NestedFunctionsAndClosures) {
    ...
    try {
        ...
        std::cout << "Nested functions or closures not fully supported: " << e.what() << std::endl;
...
TEST_F(AdvancedRhoLanguage, ArrayCreationAndManipulation) {
    ...
    try {
        ...
        std::cout << "Array operations not fully supported: " << e.what() << std::endl;
...
TEST_F(AdvancedRhoLanguage, ObjectCreationAndPropertyAccess) {
    ...
    try {
        ...
        std::cout << "Object operations not fully supported: " << e.what() << std::endl;
        GTEST_SKIP() << "Object operations not supported";
    }
    
    std::cout << "Object creation and property access test completed" << std::endl;
}
...
TEST_F(AdvancedRhoLanguage, PiIntegration) {
    ...
    try {
        ...
        std::cout << "Pi integration not fully supported: " << e.what() << std::endl;
...
TEST_F(AdvancedRhoLanguage, HigherOrderFunctions) {
    ...
    try {
        ...
        std::cout << "Higher-order functions not fully supported: " << e.what() << std::endl;
...
TEST_F(AdvancedRhoLanguage, StringManipulation) {
    ...
    try {
        ...
        try {
            ExecuteAndVerify<int>("length", 13);  // "Hello, World!" has 13 characters
        } catch (...) {
            std::cout << "String length not supported" << std::endl;
        }
    }
    catch (const std::exception& e) {
        std::cout << "String manipulation not fully supported: " << e.what() << std::endl;
...
TEST_F(AdvancedRhoLanguage, LexicalScopingAndVariableShadowing) {
    ...
    try {
        ...
        try {
            ...
            std::cout << "Block scoping variable access not supported" << std::endl;
        }
        
        // Check if the block affected the outer variable
        // This will tell us if the language uses lexical or dynamic scoping
        try {
            // If after_block_x is 20, the block modified the outer variable (dynamic scoping)
            // If after_block_x is 10, the block had its own scope (lexical scoping)
            int afterBlockX = ConstDeref<int>(exec_->GetRegistry()->GetValue(Label("after_block_x")));
            std::cout << "Block scoping: after_block_x = " << afterBlockX
                     << " (this indicates " << (afterBlockX == 10 ? "lexical" : "dynamic") << " scoping)" 
                     << std::endl;
        } catch (...) {
    ...
    std::cout << "Lexical scoping and variable shadowing test completed" << std::endl;
}