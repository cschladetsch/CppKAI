...
TEST(SimpleTest, BasicPi) {
    ...
    ASSERT_TRUE(a.GetRegistry() != nullptr) << "First integer has null registry";
    ASSERT_TRUE(b.GetRegistry() != nullptr) << "Second integer has null registry";
    ...
    ASSERT_TRUE(stack->Top().IsType<int>()) << "Pi execution result is not an int, but: " 
                                           << stack->Top().GetClass()->GetName();
    ASSERT_EQ(ConstDeref<int>(stack->Top()), 5) << "Pi execution result is not 5, but: " 
                                              << stack->Top().ToString();
