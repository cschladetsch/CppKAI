...
// This helps with continuations and ensures primitive values are extracted correctly
// Enhanced UnwrapStackValues function that works with our direct evaluation solution
// This should be unnecessary for most cases now, but it's kept for backward compatibility
inline void UnwrapStackValues(kai::Stack* stack, kai::Executor* exec) {
    if (!stack || stack->Empty()) {
        return;  // Nothing to do
    }

    // With our enhanced translator implementation, most values should already be 
    // primitive types, but we'll keep this as a safety net for complex cases
    ...
    if (stack->Size() >= 1) {
        ...
        if (topObj.IsType<Continuation>()) {
                ...
                if (tempStack.Exists()) {
                    ...
                    if (!tempStack->Empty()) {
                        ...
                        if (result.IsType<int>() || result.IsType<float>() || 
                            result.IsType<bool>() || result.IsType<String>()) {
                            // Replace the continuation with the calculated value
                            stack->Pop();
                            stack->Push(result);
            ...
            if (cont->GetCode().Valid() && cont->GetCode().Exists()) {
                ...
                // [val1, val2, op] or [ContinuationBegin, val1, val2, op, ContinuationEnd]
                else if (registry && 
                         ((code->Size() == 3 && code->At(2).IsType<Operation>()) ||
                          (code->Size() == 5 && 
                           code->At(0).IsType<Operation>() && 
                    ...
                    if (val1.IsType<int>() && val2.IsType<int>()) {
                        ...
                        switch (op) {
                            ...
                            case Operation::Divide:
                                if (num2 != 0) result = registry->New<int>(num1 / num2);
                                break;
                    ...
                    // Handle float operations
                        ...
                        switch (op) {
                            ...
                            case Operation::Multiply:
                                ...
                                break;
                            case Operation::Divide:
                                if (num2 != 0.0f) result = registry->New<float>(num1 / num2);
                                break;
                            case Operation::Greater:
                                result = registry->New<bool>(num1 > num2);
                                break;
                ...
                // Pattern 3: Special stack operations with values (dup, swap, etc.)
                // [val, Operation::Dup, Operation::Plus] or similar
                    ...
                    if (code->Size() == 3 && 
                        ...
                        code->At(2).IsType<Operation>() &&
                        ConstDeref<Operation>(code->At(1)).GetTypeNumber() == Operation::Dup &&
                        ConstDeref<Operation>(code->At(2)).GetTypeNumber() == Operation::Plus) {
                        
                        int val = ConstDeref<int>(code->At(0));
                        Object result = registry->New<int>(val + val); // Effectively doubles the value
                    ...
                    // Handle Dup + Multiply pattern (duplicates value and multiplies)
                    else if (code->Size() == 3 && 
                             code->At(0).IsType<int>() && 
                             code->At(1).IsType<Operation>() && 
                             code->At(2).IsType<Operation>() &&
                             ConstDeref<Operation>(code->At(1)).GetTypeNumber() == Operation::Dup &&
                             ConstDeref<Operation>(code->At(2)).GetTypeNumber() == Operation::Multiply) {
                        
                        int val = ConstDeref<int>(code->At(0));
                        Object result = registry->New<int>(val * val); // Effectively squares the value
