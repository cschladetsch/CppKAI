...
TEST_F(PiFloatParsingTest, LexerFloatParsing) {
    ...
    for (const auto& tok : tokens) {
        std::cout << "  Token type: " << kai::PiTokenEnumType::ToString(tok.type) 
             << ", text: '" << tok.Text() << "'" << std::endl;
    }
    
    // Should have 2 tokens: Float and None (terminator)
    ASSERT_GE(tokens.size(), 2);
    ASSERT_EQ(tokens[0].type, kai::PiTokenEnumType::Float) << "First token should be Float";
    ASSERT_EQ(tokens[0].Text(), "3.14") << "Float token should have text '3.14'";
}
...
TEST_F(PiFloatParsingTest, TranslatorFloatHandling) {
    ...
    ASSERT_FLOAT_EQ(kai::ConstDeref<float>(stack->Top()), 3.14f) << "Value should be 3.14";
}