...
TEST_F(PiBacktickComplex2Test, RecursiveShellPattern) {
    ...
    console_.Execute("`echo 5` 1 { over 1 > { over * swap 1 - swap } { } ife dup 1 > } while drop");
    auto stack = exec->GetDataStack();
    ASSERT_EQ(stack->Size(), 1);
...
TEST_F(PiBacktickComplex2Test, ShellInMapOps) {
    ...
    console_.Execute("{ `echo foo` `echo 100` `echo bar` `echo 200` } tomap 'mymap #");
...
TEST_F(PiBacktickComplex2Test, ShellTypeConversions) {
    ...
    console_.Execute("`echo 3.14` `echo 2` * to_str \" = \" + `echo 6.28` to_str +");
    auto stack = exec->GetDataStack();
    ASSERT_EQ(stack->Size(), 1);
...
TEST_F(PiBacktickComplex2Test, ParallelShellOps) {
    ...
    console_.Execute("[ `echo 1` `echo 2` `echo 3` ] [ `echo 4` `echo 5` `echo 6` ] { + } zip2");
    auto stack = exec->GetDataStack();
...
TEST_F(PiBacktickComplex2Test, ShellExceptionPattern) {
    ...
    console_.Execute("`echo 10` `echo 0` dup 0 == { drop drop \"Error: Division by zero\" } { / } ife");
    auto stack = exec->GetDataStack();
    ASSERT_EQ(stack->Size(), 1);
    EXPECT_EQ(kai::ConstDeref<kai::String>(stack->Top()), "Error: Division by zero");
}
...
TEST_F(PiBacktickComplex2Test, ComplexDataShell) {
    ...
    console_.Execute("[ [ `echo 1` `echo 2` ] [ `echo 3` `echo 4` ] ] 0 { 0 { + } fold + } fold");
    auto stack = exec->GetDataStack();
    ASSERT_EQ(stack->Size(), 1);
