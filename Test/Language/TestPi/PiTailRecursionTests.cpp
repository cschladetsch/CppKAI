...
TEST_F(PiTailRecursionTests, CountdownZero) {
    AssertResult<int>("{ dup 0 == { } { 1 - countdown ! } ife } 'countdown # 0 countdown &", 0);
}

TEST_F(PiTailRecursionTests, CountdownOne) {
    AssertResult<int>("{ dup 0 == { } { 1 - countdown ! } ife } 'countdown # 1 countdown &", 0);
}

TEST_F(PiTailRecursionTests, CountdownFive) {
    AssertResult<int>("{ dup 0 == { } { 1 - countdown ! } ife } 'countdown # 5 countdown &", 0);
}

TEST_F(PiTailRecursionTests, CountdownTen) {
    AssertResult<int>("{ dup 0 == { } { 1 - countdown ! } ife } 'countdown # 10 countdown &", 0);
}

// Tail recursive sum with accumulator
TEST_F(PiTailRecursionTests, TailSumZero) {
    // sum(0, 0) = 0
    AssertResult<int>("{ swap dup 0 == { drop } { swap over + swap 1 - sum ! } ife } 'sum # 0 0 sum &", 0);
}
...
TEST_F(PiTailRecursionTests, IsEvenZero) {
    AssertResult<bool>("{ dup 0 == { drop true } { dup 1 == { drop false } { 2 - isEven ! } ife } ife } 'isEven # 0 isEven &", true);
}

TEST_F(PiTailRecursionTests, IsEvenOne) {
    AssertResult<bool>("{ dup 0 == { drop true } { dup 1 == { drop false } { 2 - isEven ! } ife } ife } 'isEven # 1 isEven &", false);
}
