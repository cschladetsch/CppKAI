...
TEST_F(TestPiContinuation, SimpleContinuationCreation) {
    ...
    auto top = stack->Top();
    EXPECT_TRUE(top.IsType<Continuation>()) << "Expected a Continuation object on stack, got: " 
    ...
    if (top.IsType<Continuation>()) {
        ...
        // The continuation should contain: ContinuationBegin, 100, ContinuationEnd
        // Or at least contain the value 100
        bool found100 = false;
...
TEST_F(TestPiContinuation, ContinuationExecution) {
    ...
    EXPECT_TRUE(result.IsType<int>()) << "Expected int after executing continuation";
