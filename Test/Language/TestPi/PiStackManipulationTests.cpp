...
struct PiStackTests : TestLangCommon
{
    ...
    void AssertStackResult(const char *script, std::tuple<Ts...> expected, bool verbose = false)
    {
        if (verbose)
        {
    ...
    void VerifyStack(Stack &stack, std::tuple<Ts...> expected, std::index_sequence<Is...>)
    {
        ...
        {
            // Verify stack values in reverse order (top of stack is last element)
            ...
            auto checkElement = [&](size_t index, auto expectedValue) {
                ...
                if (stackIndex < stackSize) {
                    ...
                    if (val.GetType() != Type::Traits<ExpectedType>::TypeNumber) {
                        ADD_FAILURE() << "Type mismatch at stack position " << index
                                     << ". Expected: " << Type::Traits<ExpectedType>::TypeNumber
                                     << ", Got: " << val.GetType();
                    } else {
                        ExpectedType actual = kai_cast<ExpectedType>(val);
                        EXPECT_EQ(actual, expectedValue) << "Value mismatch at stack position " << index;
                    }
                }
            };
...
TEST_F(PiStackTests, CompositeStackOperations)
{
    ...
    AssertStackResult("1 2 3 dup rot swap over", std::make_tuple(1, 3, 2, 3, 3));
    AssertStackResult("1 2 3 4 1 pick 2 pick swap", std::make_tuple(1, 2, 3, 4, 2, 3));
}
...
TEST_F(PiStackTests, MixedTypeOperations)
{
    ...
    AssertStackResult("1 \"hello\" over", std::make_tuple(1, String("hello"), 1));
    
    // Mixed types
    AssertStackResult("1 2.5 true \"hello\" rot", std::make_tuple(1, true, String("hello"), 2.5f));
}
...
TEST_F(PiStackTests, ConditionalStackOperations)
{
    ...
    AssertStackResult("1 2 true { swap } { dup } ifelse", std::make_tuple(2, 1));
    AssertStackResult("1 2 false { swap } { dup } ifelse", std::make_tuple(1, 2, 2));
