...
TEST_F(TestIdentifierDebug, TestMultipleIdentifiers) {
    ...
    if (data_->Size() > 0) {
        auto top = data_->Top();
        if (top.IsType<int>()) {
            std::cout << "  Top value: " << kai::ConstDeref<int>(top) << std::endl;
        }
    }
    ...
    ASSERT_EQ(kai::ConstDeref<int>(val3), 10) << "Third value should be 10 (second x)";
    
    auto val2 = data_->Pop();
    ASSERT_TRUE(val2.IsType<int>());
    ASSERT_EQ(kai::ConstDeref<int>(val2), 20) << "Second value should be 20 (y)";
    
    auto val1 = data_->Pop();
    ASSERT_TRUE(val1.IsType<int>());
    ASSERT_EQ(kai::ConstDeref<int>(val1), 10) << "First value should be 10 (first x)";
}
...
TEST_F(TestIdentifierDebug, TestSingleExecute) {
    ...
    std::cout << "After single execute, stack size: " << data_->Size() << std::endl;
    ...
    for (int i = 0; i < 3; i++) {
        auto val = data_->At(i);
        if (val.IsType<int>()) {
            std::cout << "  Stack[" << i << "]: " << kai::ConstDeref<int>(val) << std::endl;
        }
    }
}