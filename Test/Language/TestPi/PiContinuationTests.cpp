...
{
    // Define a custom 'dip' combinator that applies a quotation to the second element
    AssertResult<int>(
        ...
        "5 10 { 2 * } 'dip' @ call",  // apply dip to multiply 5 by 2, with 10 on top
        10);  // top of stack is still 10, but 5 became 10
    
    // Define a custom 'keep' combinator that applies a quotation but keeps the original
    AssertResult<int>(
        "{ 'fn' ! "  // store the quotation
        ...
        "5 { 2 * } 'keep' @ call +",  // apply keep to multiply 5 by 2, then add to original 5
...
TEST_F(PiContinuationTests, RecursiveContinuations)
{
    ...
    AssertResult<int>(
        ...
        "  { dup 1 - 'factorial' @ call * } "  // recursive case: n * factorial(n-1)
        "  ifelse "
...
TEST_F(PiContinuationTests, StateMachines)
{
    ...
    AssertResult<int>(
        ...
        "{ } 5 'process-state' @ call "  // initial state is 1, so 5*2 = 10, new state is 2
        "{ } 'process-state' @ call "    // state is 2, so 10+3 = 13, new state is 1
