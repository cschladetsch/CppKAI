...
protected:
    ...
    string LoadScript(const char* path) {
        stringstream ss;
        ss << "/home/xian/local/KAI/Test/Language/TestTau/Scripts/Connection/" << path;
    ...
    void VerifyTauSyntax(const string& script, const string& name) {
        ...
        // Parsing might fail in current implementation, but we want to see how far it gets
        if (parseResult) {
...
TEST_F(TauNetworkTest, CombinedInterfaces) {
    string combined = 
        LoadScript("ConnectionBasic.tau") + "\n" +
