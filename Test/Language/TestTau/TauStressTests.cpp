...
class TauStressTests : public ::testing::Test {
   protected:
    ...
    string GenerateLargeInterface(int numMethods, int numEvents,
                                  const string& namePrefix = "Stress") {
        stringstream ss;
        ...
        for (int i = 0; i < numMethods; ++i) {
            if (i % 4 == 0) {
                ...
                ss << "        DataStruct Method" << i
                   << "(DataStruct input, int[] filters);\n";
            } else {
                // Method with very complex parameters
                ss << "        ComplexStruct[] Method" << i
                   << "(ComplexStruct[] inputs, DataStruct filter, bool "
                      "sorted);\n";
            }
        }
        ...
        for (int i = 0; i < numEvents; ++i) {
            if (i % 3 == 0) {
                ...
                ss << "        event Event" << i
                   << "(DataStruct data, ComplexStruct context);\n";
            }
        }
    ...
    string GenerateMultipleInterfaces(int numInterfaces,
                                      int methodsPerInterface) {
        stringstream ss;
        ...
        for (int i = 0; i < numInterfaces; ++i) {
            ...
            for (int j = 0; j < methodsPerInterface; ++j) {
                ss << "        SharedStruct Method" << j
                   << "(SharedStruct input" << j << ");\n";
            }
    ...
    bool GenerateAndMeasure(const string& tauContent, string& proxyOutput,
                            string& agentOutput, double& parseTime,
                            double& proxyTime, double& agentTime) {
        auto start = high_resolution_clock::now();

        // Parse
        try {
            auto lexer =
                make_shared<tau::TauLexer>(tauContent.c_str(), registry);
            if (!lexer->Process()) {
                return false;
            }
        ...
        parseTime =
            duration_cast<microseconds>(parseEnd - start).count() / 1000.0;
        ...
        proxyTime =
            duration_cast<microseconds>(proxyEnd - parseEnd).count() / 1000.0;
        ...
        agentTime =
            duration_cast<microseconds>(agentEnd - proxyEnd).count() / 1000.0;
...
TEST_F(TauStressTests, ScalabilityTest) {
    ...
    cout << "Methods\tEvents\tParse(ms)\tProxy(ms)\tAgent(ms)\tProxy "
            "Size\tAgent Size\n";
    cout << "-------\t------\t---------\t---------\t---------\t----------\t----"
            "------\n";
    ...
    for (size_t i = 0; i < methodCounts.size(); ++i) {
        ...
        string tauContent =
            GenerateLargeInterface(methods, events, "Scale" + to_string(i));
        ...
        EXPECT_TRUE(success) << "Failed to generate for " << methods
                             << " methods, " << events << " events";

        if (success) {
            cout << methods << "\t" << events << "\t" << fixed
                 << setprecision(2) << parseTime << "\t\t" << proxyTime
                 << "\t\t" << agentTime << "\t\t" << proxyOutput.size()
                 << "\t\t" << agentOutput.size() << "\n";

            // Verify output contains expected content
            EXPECT_TRUE(proxyOutput.find("Scale" + to_string(i)) !=
                        string::npos);
            EXPECT_TRUE(agentOutput.find("Scale" + to_string(i)) !=
                        string::npos);

            // Performance expectations (generous to account for system
            // variance)
            EXPECT_LT(parseTime, 1000.0)
                << "Parse time too slow for " << methods << " methods";
            EXPECT_LT(proxyTime, 2000.0)
                << "Proxy generation too slow for " << methods << " methods";
            EXPECT_LT(agentTime, 2000.0)
                << "Agent generation too slow for " << methods << " methods";
        }
    }
}
...
TEST_F(TauStressTests, MemoryUsageTest) {
    ...
    string tauContent =
        GenerateLargeInterface(LARGE_METHOD_COUNT, LARGE_EVENT_COUNT, "Memory");
    string proxyOutput, agentOutput;
    double parseTime, proxyTime, agentTime;
    ...
    if (success) {
        ...
        cout << "Methods: " << LARGE_METHOD_COUNT
             << ", Events: " << LARGE_EVENT_COUNT << "\n";
...
TEST_F(TauStressTests, ConcurrentGenerationTest) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        futures.push_back(async(launch::deferred, [i, &successfulGenerations,
                                                   &totalAttempts, this]() {
            cout << "Starting thread " << i << endl;
            Registry localRegistry;
            string tauContent =
                GenerateLargeInterface(100, 50, "Concurrent" + to_string(i));
            ...
            try {
                ...
                if (proxyOutput.size() > 1000 && agentOutput.size() > 1000 &&
                    proxyOutput.find("Concurrent" + to_string(i)) !=
                        string::npos &&
                    agentOutput.find("Concurrent" + to_string(i)) !=
                        string::npos) {
                    successfulGenerations++;
                    cout << "Thread " << i << " succeeded" << endl;
                    return true;
                } else {
                    cout << "Verification failed for thread " << i
    ...
    EXPECT_GE(successfulGenerations.load(),
              NUM_THREADS * 0.8);  // Allow some failures due to concurrency

    cout << "\nConcurrent Generation Test Results:\n";
    cout << "Threads: " << NUM_THREADS << "\n";
    cout << "Successful: " << successfulGenerations.load() << "/"
         << totalAttempts.load() << "\n";
}
...
TEST_F(TauStressTests, MultipleInterfacesTest) {
    ...
    for (int interfaceCount : interfaceCounts) {
        string tauContent =
            GenerateMultipleInterfaces(interfaceCount, METHODS_PER_INTERFACE);
        string proxyOutput, agentOutput;
        double parseTime, proxyTime, agentTime;
        ...
        EXPECT_TRUE(success)
            << "Failed to generate " << interfaceCount << " interfaces";

        if (success) {
            size_t totalSize = (proxyOutput.size() + agentOutput.size()) / 1024;

            cout << interfaceCount << "\t\t" << fixed << setprecision(2)
                 << parseTime << "\t\t" << proxyTime << "\t\t" << agentTime
                 << "\t\t" << totalSize << "\n";
            ...
            double expectedTime =
                interfaceCount * 10.0;  // 10ms per interface baseline
            EXPECT_LT(parseTime + proxyTime + agentTime,
                      expectedTime * 3);  // Allow 3x overhead
        }
    }
}
...
TEST_F(TauStressTests, ErrorHandlingStressTest) {
    vector<string> malformedInputs = {
        ...
        "namespace Bad { interface IBad { " + string(1000, 'v') +
            "oid method() } }",

        // Deeply nested but broken structure
        "namespace A { namespace B { namespace C { interface I { void m(int "
        "int int); } } }",

        // Very long identifier names
        "namespace Test { interface I" + string(1000, 'X') +
            " { void method(); } }",
    ...
    for (size_t i = 0; i < malformedInputs.size(); ++i) {
        ...
        bool success =
            GenerateAndMeasure(malformedInputs[i], proxyOutput, agentOutput,
                               parseTime, proxyTime, agentTime);
        ...
        EXPECT_LT(parseTime, 1000.0)
            << "Error handling took too long for input " << i;
    }
