...
public:
    ...
    bool SendMessage(const string& message) {
        ...
        if (isHosting_) {
            hostedChannels_[currentChannel_].messages[GetTimestamp()] = formattedMsg;
        } else {
            // In real P2P, this would send to channel host
            discoveredChannels_[currentChannel_].messages[GetTimestamp()] = formattedMsg;
        }
    ...
    void SimulateChannelDiscovery(const string& channelName, const string& hostUsername) {
        Channel channel;
        channel.name = channelName;
...
protected:
    ...
    void RegisterChatCommands() {
        ...
        console_->RegisterCommand("username", [this](const vector<string>& args) {
            if (args.size() != 2) {
        ...
        console_->RegisterCommand("publish", [this](const vector<string>& args) {
            if (args.size() != 2) {
                return string("Usage: publish <channel>");
            }
            
            if (!chat_->HasUsername()) {
                return string("Error: Set username first with 'username <name>'");
            }
        ...
        console_->RegisterCommand("discover", [this](const vector<string>& args) {
