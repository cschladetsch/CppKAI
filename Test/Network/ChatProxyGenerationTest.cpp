...
protected:
    ...
    string LoadTauInterface(const string& filename) {
        string path = "/home/xian/local/KAI/Test/Language/TestTau/Scripts/Connection/" + filename;
        ifstream file(path);
        if (!file.is_open()) {
...
TEST_F(ChatProxyGenerationTest, ConsoleIntegrationWithGeneratedCode) {
    // This test demonstrates how the generated proxy/agent would work with Console
...
TEST_F(ChatProxyGenerationTest, WindowIntegrationConcept) {
    ...
    struct MockChatWindow {
        ...
        void UpdatePeerList(const vector<string>& peers) {
            peerList = peers;
        }
