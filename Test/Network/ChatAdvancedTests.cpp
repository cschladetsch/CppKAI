...
struct AdvancedChatMessage {
    ...
    AdvancedChatMessage(const string& channel, const string& sender, const string& msg)
        : id(GenerateId()), type(AdvancedMessageType::Text), 
          channelId(channel), senderId(sender), senderName(sender),
          content(msg), timestamp(GetTimestamp()) {}
...
public:
    ...
    vector<AdvancedChatMessage> LoadMessages(const string& channelId, int limit = 100) {
...
public:
    ...
    function<void(string, string, string)> onMessageEdited; // messageId, channelId, newContent
    function<void(string, string)> onMessageDeleted; // messageId, channelId
    function<void(string, string, string)> onReactionAdded; // messageId, userId, emoji
    ...
    string CreateGroup(const string& name, const string& description, bool isPrivate = false) {
        GroupChatInfo group;
        ...
        group.createdTime = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    bool LeaveGroup(const string& groupId) {
        ...
        auto& group = it->second;
        auto memberIt = find(group.memberIds.begin(), group.memberIds.end(), userId);
        if (memberIt == group.memberIds.end()) return false;
        
        group.memberIds.erase(memberIt);
        
        // Remove from admins if applicable
        auto adminIt = find(group.adminIds.begin(), group.adminIds.end(), userId);
    ...
    bool AddGroupMember(const string& groupId, const string& newUserId) {
        ...
        auto& group = it->second;
        if (!group.IsAdmin(userId)) return false; // Only admins can add members
    ...
    bool RemoveGroupMember(const string& groupId, const string& targetUserId) {
        ...
        auto& group = it->second;
        if (!group.IsAdmin(userId)) return false; // Only admins can remove members
        if (targetUserId == group.creatorId) return false; // Cannot remove creator
        
        auto memberIt = find(group.memberIds.begin(), group.memberIds.end(), targetUserId);
        if (memberIt == group.memberIds.end()) return false;
    ...
    string SendMessage(const string& channelId, const string& content, 
        ...
        AdvancedChatMessage msg;
        msg.id = "msg_" + to_string(chrono::steady_clock::now().time_since_epoch().count());
        msg.type = type;
        msg.channelId = channelId;
        msg.senderId = userId;
    ...
    string ReplyToMessage(const string& channelId, const string& replyToId, const string& content) {
        AdvancedChatMessage msg;
        msg.id = "msg_" + to_string(chrono::steady_clock::now().time_since_epoch().count());
    ...
    bool EditMessage(const string& channelId, const string& messageId, const string& newContent) {
        auto it = messageCache.find(channelId);
        if (it == messageCache.end()) return false;
        
        auto msgIt = find_if(it->second.begin(), it->second.end(),
            [&](const AdvancedChatMessage& m) { return m.id == messageId; });
            
        if (msgIt == it->second.end()) return false;
        if (msgIt->senderId != userId) return false; // Can only edit own messages
        
        msgIt->content = newContent;
        msgIt->metadata.isEdited = true;
        msgIt->metadata.editTimestamp = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    bool AddReaction(const string& channelId, const string& messageId, const string& emoji) {
        auto it = messageCache.find(channelId);
        if (it == messageCache.end()) return false;
    ...
    vector<AdvancedChatMessage> SearchMessages(const string& channelId, const string& query) {
    ...
    vector<AdvancedChatMessage> GetMessagesByUser(const string& channelId, const string& targetUserId) {
        vector<AdvancedChatMessage> results;
        auto it = messageCache.find(channelId);
        if (it == messageCache.end()) return results;
    ...
    vector<AdvancedChatMessage> GetMessages(const string& channelId) const {
        auto it = messageCache.find(channelId);
        return (it != messageCache.end()) ? it->second : vector<AdvancedChatMessage>();
    }
};
...
TEST_F(ChatAdvancedTests, GroupCreation) {
    ...
    // Create a group
    string groupId = chat->CreateGroup("Test Group", "A test group for unit tests");
...
TEST_F(ChatAdvancedTests, AdvancedMessaging) {
    ...
    string emojiId = chat->SendMessage(groupId, "😀", Chat::AdvancedMessageType::Emoji);
...
TEST_F(ChatAdvancedTests, ReplyToMessage) {
    ...
    // Reply to it
    string replyId = chat->ReplyToMessage(groupId, originalId, "This is a reply");
...
TEST_F(ChatAdvancedTests, MessageEditing) {
    ...
    string newContent;
    chat->onMessageEdited = [&](string msgId, string channelId, string content) {
...
TEST_F(ChatAdvancedTests, GroupPersistence) {
    ...
    auto group1 = find_if(loadedGroups.begin(), loadedGroups.end(),
...
TEST_F(ChatAdvancedTests, ComplexMultiUserScenario) {
    ...
    chat->SetUser("charlie_id", "Charlie");
    // Note: In real implementation, would need to promote Charlie to admin first
}