...
struct MockChatMessage {
    ...
    MockChatMessage(const string& channel, const string& sender, const string& msg)
        : type(0), channelName(channel), senderId(sender), senderName(sender), 
          content(msg), timestamp(duration_cast<milliseconds>(
...
struct MockChannelInfo {
    ...
    MockChannelInfo(const string& name, const string& host)
        : channelName(name), hostId(host), hostAddress("127.0.0.1"),
          createdTime(duration_cast<milliseconds>(
              system_clock::now().time_since_epoch()).count()),
          memberCount(1), isPublic(true) {}
};
...
struct MockChannelUser {
    ...
    MockChannelUser(const string& name, bool host = false)
        : username(name), userId(name + "_id"), address("127.0.0.1"),
...
public:
    ...
    bool Enter(const string& channelName) {
        ...
        auto it = find_if(discoveredChannels.begin(), discoveredChannels.end(),
            [&](const MockChannelInfo& info) { return info.channelName == channelName; });
    ...
    vector<MockChannelInfo> Discover() {
    ...
    void SimulateUserJoin(const string& newUser) {
        ...
        // Update channel info
        auto it = find_if(discoveredChannels.begin(), discoveredChannels.end(),
            [this](const MockChannelInfo& info) { return info.channelName == currentChannel; });
...
TEST_F(ChatFunctionalityTests, ChannelClosure) {
    ...
    chat1->onChannelClosed = [&](string channel) {
