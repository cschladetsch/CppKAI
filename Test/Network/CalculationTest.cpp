...
protected:
    ...
    void BuildExecutables() {
        ...
        std::string buildCommand = "cd " + buildDir + " && cmake --build . --target ConfigurableServer ConfigurableClient";
    ...
    void StartServer() {
        ...
        if (serverPid == 0) {
            ...
            execl(serverPath.c_str(), serverPath.c_str(), configPath.c_str(), nullptr);
    ...
    int RunClient(const std::string& expression) {
        ...
        if (clientPid == 0) {
            ...
            execl(clientPath.c_str(), clientPath.c_str(), configPath.c_str(), nullptr);
...
TEST_F(CalculationTest, BasicAddition) {
    ...
    ASSERT_EQ(exitCode, 0) << "Client did not return success for calculation 1+2=3";
...
TEST_F(CalculationTest, InvalidExpression) {
    ...
    ASSERT_NE(exitCode, 0) << "Client incorrectly returned success for unsupported calculation";
}
