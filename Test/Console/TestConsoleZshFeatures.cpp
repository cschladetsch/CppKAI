...
protected:
    ...
    void AddToHistory(const std::string& cmd) {
        ...
        // Since commandHistory is private, we'll need to execute through the full path
        SimulateCommand(cmd);
    }
...
TEST_F(ConsoleZshTest, DoubleExclamationRepeatsLastCommand) {
    ...
    // In practice, you might need to make it public for testing or use friend class
...
TEST_F(ConsoleZshTest, MultipleHistoryReferences) {
    ...
    kai::String result = console.ExpandHistoryReferences(kai::String("!-2 + !-1"));
    EXPECT_EQ(result.StdString(), "10 + 20");
}

// Test 14: Shell command with backticks
TEST_F(ConsoleZshTest, BacktickShellCommand) {
    kai::String result = console.ProcessShellCommand(kai::String("`echo hello`"));
...
TEST_F(ConsoleZshTest, EmbeddedShellCommands) {
    kai::String result = console.ExpandShellCommands(kai::String("The date is `date +%Y`"));
    EXPECT_TRUE(result.StdString().find("202") != std::string::npos); // Year starts with 202x
}
