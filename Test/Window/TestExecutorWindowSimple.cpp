...
// Since we can't directly test the UI components without the actual imgui implementation,
// we'll test the core console functionality that the Window uses
...
TEST_F(WindowConsoleTest, Pi_ErrorHandling) {
    ...
    EXPECT_THROW(console.Execute("undefined_var", Structure::Expression), Exception::Base);
    
    // Test division by zero
    EXPECT_THROW(console.Execute("1 0 /", Structure::Expression), Exception::Base);
}
...
TEST_F(WindowConsoleTest, Rho_ControlFlow) {
    ...
    console.Execute("if (x > 3) { \"greater\" } else { \"less\" }", Structure::Expression);
    auto stack = console.GetExecutor()->GetDataStack();
    EXPECT_GT(stack->Size(), 0);
}
...
TEST_F(WindowConsoleTest, Rho_ErrorHandling) {
    ...
    EXPECT_THROW(console.Execute("undefined_variable", Structure::Expression), Exception::Base);
}
