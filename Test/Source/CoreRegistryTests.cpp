...
TEST_F(CoreRegistryTests, TestTypeRegistration) {
    ...
    // When we added required classes in AddRequiredClasses(), they should be registered
    ...
    // Test with TypeNumber directly - validate that we can get classes by their type number
...
TEST_F(CoreRegistryTests, TestClassPropertyAccess) {
    ...
    // We don't assert on the property existence, just that the method completed successfully
    // Specific property names vary by implementation
}
