...
TEST_F(CoreAdvancedTests, ComplexContainerOps) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        dict.GetClass()->Insert(dict, 
            reg.New<kai::String>("key" + std::to_string(i)), 
            innerVec);
    }
...
TEST_F(CoreAdvancedTests, BinarySerializationRoundTrip) {
    ...
    root.GetClass()->Insert(root, reg.New<kai::String>("name"), reg.New<kai::String>("test"));
    root.GetClass()->Insert(root, reg.New<kai::String>("count"), reg.New<int>(42));
...
TEST_F(CoreAdvancedTests, ThreadLocalStorage) {
    ...
    ASSERT_EQ(kai::ConstDeref<int>(threadContext.GetProperty("threadId")), 12345);
    
    ASSERT_TRUE(threadContext.HasProperty("userName"));
    ASSERT_EQ(kai::ConstDeref<kai::String>(threadContext.GetProperty("userName")), "testuser");
}
