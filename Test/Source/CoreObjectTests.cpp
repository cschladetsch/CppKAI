...
TEST_F(CoreObjectTests, TestObjectComparison) {
    ...
    // Note: This is implementation dependent - in KAI, objects with same value may have same handle
    // So we can't reliably assert inequality here
    // ASSERT_FALSE(intObj1 == intObj2);    // Different handles even though same value
...
TEST_F(CoreObjectTests, TestObjectWrapperMethods) {
    ...
    // The label's text might vary by implementation (e.g., "String" vs "kai::String")
