...
struct ChatMessage {
    ...
    ChatMessage(MessageType t, const std::string& ch, const std::string& s, const std::string& c)
...
public:
    ...
    bool Enter(const std::string& channelName) {
        ...
        // Notify others
        ChatMessage joinMsg(MessageType::Join, channelName, username_, username_ + " joined the channel");
        if (onMessageReceived_) onMessageReceived_(joinMsg);
        if (onUserJoined_) onUserJoined_(username_);
        
        std::cout << "[SYSTEM] " << username_ << " joined channel '" << channelName << "'\n";
        return true;
    }
    ...
    void Send(const std::string& message) {
        if (currentChannel_.empty()) {
            std::cout << "[ERROR] Not in any channel. Use 'publish' or 'enter' first.\n";
            return;
        }
    ...
    void Leave() {
        ...
        std::cout << "[SYSTEM] " << username_ << " left channel '" << currentChannel_ << "'\n";
        
        // Notify others
        ChatMessage leaveMsg(MessageType::Leave, currentChannel_, username_, username_ + " left the channel");
        ...
        if (isHost_) {
            ...
            std::cout << "[SYSTEM] Channel '" << currentChannel_ << "' closed (host left)\n";
        } else {
            // Update member count
            ...
            if (cmd == "help") {
                ...
                if (currentChannel_.empty()) {
                    ...
                    for (const auto& user : userList) {
                        std::cout << "  - " << user;
                        if (isHost_ && user == username_) std::cout << " (host)";
                        std::cout << "\n";
                    }
                    std::cout << "\n";
                }
