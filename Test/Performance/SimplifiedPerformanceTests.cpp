...
TEST_F(SimplifiedPerformanceTests, TauParsingPerformance) {
    ...
    auto parseResult =
        BenchmarkFunction([&]() { ParseTauCode(simpleInterface); }, 1000);
    ...
    auto complexParseResult =
        BenchmarkFunction([&]() { ParseTauCode(complexInterface); }, 500);
...
TEST_F(SimplifiedPerformanceTests, ScalabilityBenchmark) {
    ...
    for (int methodCount : methodCounts) {
        ...
        // Benchmark parsing
        auto parseTime =
            BenchmarkFunction([&]() { ParseTauCode(interfaceCode); }, 100);
        ...
        double totalTime = parseTime.averageTime + proxyTime.averageTime +
                           agentTime.averageTime;

        cout << methodCount << "\t" << fixed << setprecision(2)
             << parseTime.averageTime << "\t\t" << proxyTime.averageTime
             << "\t\t" << agentTime.averageTime << "\t\t" << totalTime << "\n";
    }
}
...
TEST_F(SimplifiedPerformanceTests, SystemThroughputBenchmark) {
    ...
    double operationsPerSecond =
        (operationsPerIteration * 1000.0) / throughputResult.averageTime;
    ...
    cout << "Average latency per operation: "
         << throughputResult.averageTime / operationsPerIteration << " ms\n";
}