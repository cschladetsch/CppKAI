   ...
   protected:
    ...
    struct BenchmarkResult {
        ...
        void print(const string& testName) const {
            ...
            cout << "Average: " << fixed << setprecision(3) << averageTime
                 << " ms\n";
            cout << "Min: " << minTime << " ms\n";
            cout << "Max: " << maxTime << " ms\n";
    ...
    BenchmarkResult BenchmarkFunction(Func func, size_t iterations = 1000) {
        ...
        for (double time : times) {
            variance +=
                (time - result.averageTime) * (time - result.averageTime);
        }
    ...
    bool ParseTauCode(const string& code) {
        try {
            ...
            auto lexer =
                make_shared<tau::TauLexer>(code.c_str(), localRegistry);
            if (!lexer->Process()) {
                return false;
            }
...
TEST_F(PerformanceBenchmarkTests, CoreObjectSystemPerformance) {
    ...
    auto compareResult = BenchmarkFunction(
        [&]() {
            ...
            (void)result1;
            (void)result2;  // Suppress warnings
        },
        2000);
...
TEST_F(PerformanceBenchmarkTests, TauCodeGenerationPerformance) {
    ...
    auto parseResult = BenchmarkFunction(
        [&]() {
            Registry localRegistry;
            auto lexer = make_shared<tau::TauLexer>(simpleInterface.c_str(),
                                                    localRegistry);
            lexer->Process();
            auto parser = make_shared<tau::TauParser>(localRegistry);
    ...
    auto proxyResult = BenchmarkFunction(
        [&]() {
            string output;
            tau::Generate::GenerateProxy generator(simpleInterface.c_str(),
                                                   output);
        },
        1000);
    ...
    auto agentResult = BenchmarkFunction(
        [&]() {
            string output;
            tau::Generate::GenerateAgent generator(simpleInterface.c_str(),
                                                   output);
        },
        1000);
    ...
    complexInterface
        << "        int id; string name; float value; bool active;\n";
    complexInterface << "    }\n";
    complexInterface << "    interface IComplex {\n";
    for (int i = 0; i < 50; ++i) {
        complexInterface << "        DataStruct Method" << i
                         << "(DataStruct param" << i << ");\n";
    }
    for (int i = 0; i < 25; ++i) {
        complexInterface << "        event Event" << i << "(DataStruct data"
                         << i << ");\n";
    }
    complexInterface << "    }\n}\n";
    ...
    auto complexParseResult = BenchmarkFunction(
        [&]() {
            Registry localRegistry;
            auto lexer = make_shared<tau::TauLexer>(complexInterfaceStr.c_str(),
                                                    localRegistry);
            lexer->Process();
            auto parser = make_shared<tau::TauParser>(localRegistry);
            parser->Process(lexer, Structure::Module);
    ...
    auto complexProxyResult = BenchmarkFunction(
        [&]() {
            string output;
            tau::Generate::GenerateProxy generator(complexInterfaceStr.c_str(),
                                                   output);
        },
        100);
    ...
    auto mixedResult = BenchmarkFunction(
        [&]() {
            ...
            for (int i = 0; i < 50; ++i) {
                ...
                auto stringObj =
                    localRegistry.New<string>("test" + to_string(i));
                (void)intObj;
                (void)floatObj;
                (void)stringObj;
            }
...
TEST_F(PerformanceBenchmarkTests, SystemThroughputBenchmark) {
    ...
    double operationsPerSecond =
        (totalOperations * 1000.0) / mixedWorkloadResult.averageTime;

    cout << "Operations per iteration: " << totalOperations << "\n";
    cout << "System throughput: " << operationsPerSecond << " ops/sec\n";
    cout << "Average latency per operation: "
         << mixedWorkloadResult.averageTime / totalOperations << " ms\n";
}