...
ConnectionManager::ConnectionId ConnectionManager::AddConnection(const RakNet::SystemAddress& address)
{
    if (!_peer)
        return 0;
...
ConnectionInfo* ConnectionManager::GetConnectionByAddress(const RakNet::SystemAddress& address)
{
    std::string addrStr = address.ToString();
    auto it = _addressToId.find(addrStr);
    if (it != _addressToId.end()) {
...
const std::unordered_map<ConnectionManager::ConnectionId, ConnectionInfo>& ConnectionManager::GetAllConnections() const
{
    return _connections;
}
...
bool ConnectionManager::IsConnected(const RakNet::SystemAddress& address) const
{
    std::string addrStr = address.ToString();
    auto it = _addressToId.find(addrStr);
    if (it != _addressToId.end()) {
...
void ConnectionManager::OnConnectionEvent(const RakNet::SystemAddress& address, ConnectionEvent event)
{
    std::string addrStr = address.ToString();
    auto it = _addressToId.find(addrStr);
    if (it != _addressToId.end()) {
