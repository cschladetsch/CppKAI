...
void PeerDiscovery::Start(int discoveryPort) 
{
    ...
    _isDiscovering = true;
    std::cout << "Started peer discovery on port " << _discoveryPort << std::endl;
}
...
const std::vector<RakNet::SystemAddress>& PeerDiscovery::GetDiscoveredPeers() const
{
    return _discoveredPeers;
}

bool PeerDiscovery::IsActive() const
{
    return _isDiscovering;
}

void PeerDiscovery::ClearDiscoveredPeers()
{
...
void PeerDiscovery::ProcessDiscoveryResponse(RakNet::Packet* packet)
{
    ...
    if (!alreadyDiscovered) {
        ...
        std::cout << "Discovered peer: " << packet->systemAddress.ToString() << std::endl;
    }
}
