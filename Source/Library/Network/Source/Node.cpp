...
void Node::Listen(int port)
{
    ...
    if (result != RakNet::RAKNET_STARTED) {
        std::cerr << "Failed to start RakNet server, error code: " << result << std::endl;
        return;
    }
...
void Node::Listen(IpAddress const &address, int port)
{
    ...
    if (result != RakNet::RAKNET_STARTED) {
        std::cerr << "Failed to start RakNet server on specific address, error code: " << result << std::endl;
        return;
    }
    ...
    std::cout << "Network node listening on " << address.ToString() << ":" << port << std::endl;
}
...
void Node::Connect(IpAddress const &ip, int port)
{
    ...
    if (!_isRunning) {
        ...
        if (result != RakNet::RAKNET_STARTED) {
            std::cerr << "Failed to start RakNet client, error code: " << result << std::endl;
            return;
        }
    ...
    std::cout << "Connecting to " << ip.ToString() << ":" << port << "..." << std::endl;
}
...
void Node::SetPeerDiscoveryCallback(std::function<void(const RakNet::SystemAddress&)> callback)
{
    if (!_peerDiscovery)
        return;
...
{
    if (!packet || !_peer || !_connectionManager)
        return;
    ...
    switch (packetType) {
        case RakNet::ID_CONNECTION_REQUEST_ACCEPTED:
            ...
            _connectionManager->OnConnectionEvent(packet->systemAddress, ConnectionEvent::Connected);
            break;
            
        case RakNet::ID_CONNECTION_ATTEMPT_FAILED:
            // Failed to connect to peer
            _connectionManager->OnConnectionEvent(packet->systemAddress, ConnectionEvent::ConnectionFailed);
            break;
        ...
        case RakNet::ID_DISCONNECTION_NOTIFICATION:
            ...
            _connectionManager->OnConnectionEvent(packet->systemAddress, ConnectionEvent::Disconnected);
            _connectionManager->RemoveConnection(packet->systemAddress);
            break;
            
        case RakNet::ID_CONNECTION_LOST:
            // A peer disconnected ungracefully
            _connectionManager->OnConnectionEvent(packet->systemAddress, ConnectionEvent::ConnectionLost);
            _connectionManager->RemoveConnection(packet->systemAddress);
            break;
            
        case RakNet::ID_USER_PACKET_ENUM + 1: // NetworkSerializer::ID_KAI_OBJECT_MESSAGE:
            // Handle KAI object message
            ProcessObjectMessage(packet);
            break;
            
        case RakNet::ID_USER_PACKET_ENUM + 2: // NetworkSerializer::ID_KAI_FUNCTION_CALL:
            // Handle KAI function call
            ProcessFunctionCall(packet);
            break;
            
        case RakNet::ID_USER_PACKET_ENUM + 3: // NetworkSerializer::ID_KAI_EVENT_NOTIFICATION:
...
void Node::OnConnectionEvent(int connectionId, ConnectionEvent event)
{
    ...
    switch (event) {
        case ConnectionEvent::Connected:
            std::cout << "Connection established with peer " << connectionId << std::endl;
            break;
            
        case ConnectionEvent::Disconnected:
            std::cout << "Peer " << connectionId << " disconnected gracefully" << std::endl;
            break;
            
        case ConnectionEvent::ConnectionLost:
            std::cout << "Connection lost with peer " << connectionId << std::endl;
            break;
            
        case ConnectionEvent::ConnectionFailed:
            std::cout << "Failed to connect to peer " << connectionId << std::endl;
            break;
...
unsigned char Node::GetPacketIdentifier(RakNet::Packet *packet)
{
    ...
    if ((unsigned char)packet->data[0] == RakNet::ID_TIMESTAMP) {
        ...
        if (packet->length > sizeof(RakNet::MessageID) + sizeof(RakNet::TimeMS))
            return (unsigned char)packet->data[sizeof(RakNet::MessageID) + sizeof(RakNet::TimeMS)];
        else
            return 255;
    }
    else {
