...
private:
    ...
    bool ParseTauFile(const std::string& content) {
        try {
            ...
            std::regex methodPattern(R"(\s*(\w+(?:<[\w\s,<>]+>)?)\s+(\w+)\s*\(([^)]*)\)\s*;)");
            ...
            while (std::getline(stream, line)) {
                ...
                // Check for class
                else if (currentNamespace && std::regex_search(line, match, classPattern)) {
                    currentNamespace->classes.push_back({match[1]});
                    currentClass = &currentNamespace->classes.back();
                }
                // Check for method
                else if (currentClass && std::regex_search(line, match, methodPattern)) {
                    Method method;
                    ...
                    if (!params.empty()) {
                        ...
                        while (std::getline(paramStream, param, ',')) {
                            ...
                            if (std::regex_search(param, paramMatch, paramPattern)) {
                                method.params.push_back({paramMatch[1], paramMatch[2]});
                            }
                        }
                    }
    ...
    std::string GenerateProxyClass(const Namespace& ns, const Class& cls) {
        ...
        ss << "public:\n";
        ss << "    " << cls.name << "Proxy(kai::net::Node* node, kai::net::NetHandle handle)\n";
        ...
        for (const auto& method : cls.methods) {
            ...
            ss << "        bs.Write((unsigned char)kai::net::NetworkSerializer::ID_KAI_FUNCTION_CALL);\n";
            ss << "        bs.Write(_handle);\n";
            ...
            if (method.returnType == "void") {
                ...
                ss << "        auto future = _node->SendWithResponse<" << method.returnType << ">(_handle, bs);\n";
    ...
    std::string GenerateAgentClass(const Namespace& ns, const Class& cls) {
        ...
        ss << "public:\n";
        ss << "    " << cls.name << "Agent(kai::net::Node* node, " << ns.name << "::" << cls.name << "* impl)\n";
        ss << "        : _node(node), _implementation(impl) {}\n\n";
        
        ss << "    void ProcessMessage(RakNet::BitStream& bs, RakNet::SystemAddress& sender) {\n";
        ...
        for (const auto& method : cls.methods) {
            ss << "    void Handle_" << method.name << "(RakNet::BitStream& bs, RakNet::SystemAddress& sender) {\n";
            ...
            if (method.returnType == "void") {
                ...
                ss << "        " << method.returnType << " result = _implementation->" << method.name << "(";
            }
            ...
            if (method.returnType != "void") {
                ...
                ss << "        response.Write((unsigned char)kai::net::NetworkSerializer::ID_KAI_FUNCTION_RESPONSE);\n";
    ...
    bool ProcessFile(const std::string& inputFile, const std::string& outputFile) {
        try {
            ...
            if (!ParseTauFile(content)) {
                std::cerr << "Failed to parse Tau file: " << inputFile << std::endl;
                return false;
            }
            ...
            std::cerr << "Error generating proxy code: " << e.what() << std::endl;
            return false;
        }
    }
};
...
void GenerateNetworkProxy()
{
    ...
    if (generator.ProcessFile(inputFile, outputFile)) {
        std::cout << "GenerateNetworkProxy: Successfully generated proxy code in " << outputFile << std::endl;
    } else {
        std::cerr << "GenerateNetworkProxy: Failed to generate proxy code" << std::endl;
    }
}
