        ...
        switch (Current().type) {
            ...
            default: {
                ...
                return Fail(Lexer::CreateErrorMessage(
                    cur, "Unexpected token %s in module scope, expected 'namespace' or 'class'",
...
bool TauParser::Namespace(AstNodePtr root) {
    ...
    if (CurrentIs(TokenEnum::Assign)) {
        ...
        // Store the alias information (even though we may not support it fully yet)
        auto aliasNode = NewNode(TauAstEnumType::TokenType, originalName->GetToken());
        ns->Add(aliasNode);
    ...
    while (!Empty() && !CurrentIs(TokenEnum::CloseBrace)) {
        switch (Current().type) {
            ...
            case TokenEnum::Ident:
                if (Current().ToString() == "using") {
                    ...
                    while (!CurrentIs(TokenEnum::Semi)) {
                        ...
                        if (CurrentIs(TokenEnum::Ident) && Current().ToString() == "namespace") {
                    ...
                    // We've handled the using directive (though we may not fully support
                    // it in the code generation yet)
                    break;
                }
            ...
            default: {
                auto const &cur = Current();
                return Fail(Lexer::CreateErrorMessage(
                    cur, "Unexpected token %s in namespace body, expected 'class', 'namespace', or 'using'",
                    TokenEnumType::ToString(cur.type)));
            }
        }
    }
...
bool TauParser::Class(AstNodePtr root) {
    ...
    if (CurrentIs(TokenEnum::Semi)) { // Using :
        ...
        // Store base class information in the AST (though we may not support inheritance fully yet)
        auto baseNode = NewNode(TauAstEnumType::Inherits, baseClassName->GetToken());
        klass->Add(baseNode);
    }
    ...
    while (!Failed && !CurrentIs(TokenEnum::CloseBrace)) {
        ...
        if (CurrentIs(TokenType::OpenParan)) {
            Consume();
            if (!Method(klass, ty->GetToken(), name->GetToken())) 
                return false;
        } 
        // Otherwise it's a field
        else {
            if (!Field(klass, ty->GetToken(), name->GetToken())) 
                return false;
        }
    }
