...
int main(int argc, char **argv) {
    ...
    if (connectResult != RakNet::CONNECTION_ATTEMPT_STARTED) {
        cerr << "Failed to start connection attempt, error code: " << connectResult << "\n";
        peer->Shutdown(0);
        RakNet::RakPeerInterface::DestroyInstance(peer);
        return 1;
    }
    
    cout << "Client running. Enter messages to send or type 'exit' to quit.\n\n";
    ...
    thread inputThread([&]() {
        while (running) {
            ...
            if (!input.empty()) {
                ...
                // Use constants defined in RakNetStub.h for packet priority and reliability
                // Note: These constants aren't defined in the stub currently, using a channel 0
                peer->Send(&bs, 0, 0, 0, 
                          RakNet::SystemAddress(), true);
    ...
    while (running) {
        ...
        while ((packet = peer->Receive()) != nullptr) {
            switch (packet->data[0]) {
                case RakNet::ID_CONNECTION_REQUEST_ACCEPTED:
                    cout << "Connected to server at " << packet->systemAddress.ToString() << endl;
                    break;
        ...
        if (++counter % 50 == 0) {
            ...
            // Note that our stub implementation doesn't have NumberOfConnections or GetSystemAddressFromIndex
            bool isConnected = peer->GetConnectionState(RakNet::SystemAddress()) == RakNet::IS_CONNECTED;
            cout << "Connected to server: " << (isConnected ? "Yes" : "No") << endl;
            
            if (isConnected) {
                // In a real implementation we would have:
                // RakNet::SystemAddress serverAddr = peer->GetSystemAddressFromIndex(0);
                RakNet::SystemAddress serverAddr; // Use default address since we can't get it from the stub
                cout << "  Server address: " << serverAddr.ToString() << endl;
                cout << "  Ping: " << peer->GetAveragePing(serverAddr) << "ms" << endl;
            }
        }
