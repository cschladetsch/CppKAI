...
// Include KAI Network with RakNet adapter (will use either real or stub implementation)
...
public:
    NetworkPeer(const PeerConfig& cfg) : config(cfg), peer(nullptr) {
        ...
        for (const auto& cmd : config.commands) {
            std::cout << "Registering predefined command: " << cmd.first << " = " << cmd.second << std::endl;
        }
    }
    ...
    void StartListening() {
        std::cout << "Starting to listen on port " << config.listenPort << std::endl;
        
        RakNet::SocketDescriptor sd(config.listenPort, nullptr);
        RakNet::StartupResult result = peer->Startup(32, &sd, 1);
        
        if (result != RakNet::RAKNET_STARTED) {
            std::cerr << "Failed to start peer listener, error code: " << result << std::endl;
            return;
        }
        
        peer->SetMaximumIncomingConnections(32);
        std::cout << "Listening for connections on port " << config.listenPort << std::endl;
    }
    ...
    void ConnectToPeer(const std::string& host, int port) {
        ...
        if (result != RakNet::CONNECTION_ATTEMPT_STARTED) {
            ...
            std::cout << "Connection attempt to " << host << ":" << port << " started" << std::endl;
        }
    }
    ...
    void ProcessMessages() {
        while (running) {
            ...
            // Only simulate for a few seconds after startup to avoid continuous simulation
            ...
            if (simulationCount < 30) {  // Simulate for about 30 iterations
                if (simulationCount == 5) {
                    ...
                    simulatedPacket->data[0] = RakNet::ID_CONNECTION_REQUEST_ACCEPTED;
                    ...
                    if (!connectedPeers.empty()) {
                        ...
                        resultPacket->data = new unsigned char[resultPacket->length];
                        memcpy(resultPacket->data, bs.GetData(), resultPacket->length);
    ...
    void HandlePacket(RakNet::Packet* packet) {
        ...
        switch (packetId) {
            case RakNet::ID_NEW_INCOMING_CONNECTION:
                std::cout << "Peer connected from " << packet->systemAddress.ToString() << std::endl;
            ...
            case RakNet::ID_DISCONNECTION_NOTIFICATION:
                std::cout << "Peer disconnected: " << packet->systemAddress.ToString() << std::endl;
                RemovePeer(packet->systemAddress);
                break;
                
            case RakNet::ID_CONNECTION_LOST:
                std::cout << "Connection lost to peer: " << packet->systemAddress.ToString() << std::endl;
                RemovePeer(packet->systemAddress);
                break;
                
            case RakNet::ID_CONNECTION_REQUEST_ACCEPTED:
                std::cout << "Connected to peer: " << packet->systemAddress.ToString() << std::endl;
                // Track the new connection
                {
                    std::lock_guard<std::mutex> lock(peersMutex);
                    connectedPeers.push_back(packet->systemAddress);
            ...
            case ID_P2P_RESULT: {
                ...
                std::cout << "Result from " << packet->systemAddress.ToString()
                          << " for command '" << command << "': " << result << std::endl;
                break;
            }
        }
    }
    
    void registerCommandProcessor(const std::string& commandType, CommandProcessor processor) {
    ...
    void SendCommandToPeer(const std::string& peerName, const std::string& command) {
        ...
        if (!found) {
            std::cout << "No peer found matching '" << peerName << "'" << std::endl;
            return;
        }
        ...
        std::cout << "Sent command '" << command << "' to " << targetPeer.ToString() << std::endl;
    }
    ...
    void ListConnectedPeers() {
        ...
        for (size_t i = 0; i < connectedPeers.size(); ++i) {
            std::cout << "  " << i << ": " << connectedPeers[i].ToString() << std::endl;
        }
    }
    ...
    void PrintHelp() {
        ...
        std::cout << "  @<peer> <command> - Execute command on remote peer" << std::endl;
        std::cout << "  calc <expression> - Calculate simple expressions" << std::endl;
    ...
    try {
        ...
        if (argc > 2) {
            try {
                ...
                std::cerr << "Invalid port number. Using configured port " << config.listenPort << std::endl;
            }
        }
