...
struct ServerConfig {
    ...
    bool loadFromFile(const std::string& filename) {
        try {
            ...
            enableCalculation = tree.get<bool>("enableCalculation", enableCalculation);
...
int main(int argc, char** argv) {
    ...
    if (argc > 2) {
        try {
            ...
            std::cerr << "Invalid port number. Using configured port " << config.port << std::endl;
        }
    }
    ...
    std::cout << "  Calculation Enabled: " << (config.enableCalculation ? "Yes" : "No") << "\n";
            ...
            switch (packet->data[0]) {
                case RakNet::ID_NEW_INCOMING_CONNECTION:
                    ...
                    
                    {
                        ...
                        bs.Write(std::string("Welcome to the KAI calculation server!"));
                ...
                case ID_CUSTOM_MESSAGE: {
                    ...
                    std::cout << "Message from " << packet->systemAddress.ToString()
                         << ": " << message << std::endl;
                    ...
                    if (config.enableCalculation && message.find("+") != std::string::npos) {
                        int result = performCalculation(message);
                        if (result != -1) {
                            // Send calculation result
                            RakNet::BitStream calcResponse;
                            calcResponse.Write((RakNet::MessageID)ID_CALCULATION_RESPONSE);
                            calcResponse.Write(std::string(message));
                            calcResponse.Write(result);
                            peer->Send(&calcResponse, 0, 0, 0, packet->systemAddress, false);
                            
                            std::cout << "Sent calculation result: " << message << " = " << result << std::endl;
                        }
                    }
                } break;
                ...
                case ID_CALCULATION_REQUEST: {
                    if (config.enableCalculation) {
                        ...
                        std::cout << "Calculation request from " << packet->systemAddress.ToString()
                             << ": " << expression << std::endl;
                        ...
                        // Send result back
                        RakNet::BitStream response;
                        response.Write((RakNet::MessageID)ID_CALCULATION_RESPONSE);
                        response.Write(expression);
                        response.Write(result);
                        peer->Send(&response, 0, 0, 0, packet->systemAddress, false);
                        
                        std::cout << "Sent calculation result: " << expression << " = " << result << std::endl;
                    }
                } break;
            }
