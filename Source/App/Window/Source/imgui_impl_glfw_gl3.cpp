...
bool ImGui_ImplGlfwGL3_Init(GLFWwindow* window, bool install_callbacks) {
    ...
    io.KeyMap[ImGuiKey_Tab] =
        GLFW_KEY_TAB;  // Keyboard mapping. ImGui will use those indices to peek
...
void ImGui_ImplGlfwGL3_NewFrame() {
    if (!g_FontTexture) ImGui_ImplGlfwGL3_CreateDeviceObjects();
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED)) {
        if (io.WantCaptureMouse) {
            ...
            io.MousePos = ImVec2(
                (float)mouse_x,
                (float)
                    mouse_y);  // Mouse position in screen coordinates (set to
