...
try {
    Write-Host "Rebuilding (Console target only, via build.py)..." -ForegroundColor Cyan
    py build.py --config Release --target Console
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Build failed (exit code $LASTEXITCODE). Fix the build error, then re-run with -SkipBuild once it's patched, or run 'py build.py --config Release --target Console' yourself."
    }
